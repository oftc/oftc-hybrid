...
struct Message map_msgtab = {
    ...
    0,          1,
    0,          MFLG_SLOW,
    0,          {m_unregistered, m_ignore, m_svscloak, m_ignore, m_ignore}};
           ...
           char *parv[])
{
    ...
    if((target_p = find_person(client_p, target)))
    {
        ...
        {
            sendto_one(target_p, ":%s NOTICE %s :Activating Cloak: %s", me.name,
                       target_p->name, hostname);
            sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                                 "Activating Cloak: %s -> %s for %s",
                                 target_p->host, hostname, target_p->name);
        }
        ...
        {
            delete_user_host(target_p->username, target_p->host,
                             !MyConnect(target_p));
