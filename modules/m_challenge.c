 ...
 */
static void
m_challenge(struct Client *client_p, struct Client *source_p, int parc,
            char *parv[])
{
    char *challenge;
    ...
    if(*parv[1] == '+')
    {
        ...
        if((conf = find_exact_name_conf(
                OPER_TYPE, source_p->localClient->auth_oper, source_p->username,
        ...
        ilog(L_TRACE, "OPER %s by %s!%s@%s", source_p->localClient->auth_oper,
    ...
    if((conf = find_conf_exact(OPER_TYPE, parv[1], source_p->username,
                               source_p->host)) != NULL)
    {
        aconf = (struct AccessItem *)map_to_conf(conf);
    }
    else if((conf = find_conf_exact(OPER_TYPE, parv[1], source_p->username,
