 ...
 *
 *   $Id: /local/oftc-hybrid/trunk/modules/soft_reboot.c 1141
 * 2006-11-23T13:37:21.703719Z stu  $
 */
 ...
 */
static void
restore_socket(struct Client *client_p, int fd, int ctrlfd, time_t first,
               time_t last)
{
    char buf[HOSTLEN + 16];
    ...
    snprintf(buf, sizeof(buf),
             IsClient(client_p) ? "Nick: %s" : (ctrlfd >= 0 ? "slink data: %s"
                                                            : "Server: %s"),
             client_p->name);
...
restore_client(struct Client *client_p, char *capabs)
{
    ...
    if(client_p->username[0] != '~')
        SetGotId(client_p);
    strlcpy(userbuf, client_p->username + !IsGotId(client_p), sizeof(userbuf));
    ...
    dlinkAdd(client_p, &client_p->localClient->lclient_node,
             &local_client_list);
...
{
    return IsMe(client_p->servptr) ? client_p
                                   : discover_from(client_p->servptr);
}
...
load_state(int transfd)
{
    ...
    while(fread(&si, sizeof(si), 1, f) == 1)
    {
        ...
        if(client_p != NULL)
            (IsServer(client_p) ? restore_server : restore_client)(client_p,
                                                                   buf);
