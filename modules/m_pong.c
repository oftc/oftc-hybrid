...
struct Message pong_msgtab = {
    ...
    0,      {mr_pong, m_ignore, ms_pong, m_ignore, m_ignore, m_ignore}};
...
static void
ms_pong(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
       ...
       irccmp(destination, me.id))
    {
        if((target_p = find_client(destination)) ||
           (target_p = find_server(destination)))
            sendto_one(target_p, ":%s PONG %s %s", parv[0], origin,
                       destination);
        else
        {
...
static void
mr_pong(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
