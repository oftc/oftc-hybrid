...
struct Message pass_msgtab = {
    ...
    0,      {mr_pass, m_registered, m_ignore, m_ignore, m_registered, mr_pass}};
 ...
 */
static void
mr_pass(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
    ...
    if(EmptyString(password))
    {
        sendto_one(client_p, form_str(ERR_NEEDMOREPARAMS), me.name,
                   EmptyString(parv[0]) ? "*" : parv[0], "PASS");
        return;
    }
