...
struct Message oper_msgtab = {
    ...
    0,      3,
    0,      MFLG_SLOW,
    0,      {m_unregistered, m_oper, m_ignore, m_ignore, mo_oper, m_ignore}};
...
m_oper(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
    ...
    if((conf = find_password_conf(name, source_p)) == NULL)
    {
        ...
        failed_oper_notice(source_p, name, (conf != NULL) ? "host mismatch"
                                                          : "no oper {} block");
    ...
    if(match_conf_password(password, aconf))
    {
        ...
        {
            sendto_one(source_p, ":%s NOTICE %s :Can't attach conf!", me.name,
                       source_p->name);
        ...
        ilog(L_TRACE, "OPER %s by %s!%s@%s", name, source_p->name,
...
*/
static void
mo_oper(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
...
find_password_conf(const char *name, struct Client *source_p)
{
    ...
    if((conf = find_exact_name_conf(OPER_TYPE, name, source_p->username,
                                    source_p->host, source_p->certfp)) != NULL)
    {
        return (conf);
    }
                   ...
                   const char *reason)
{
    if(ConfigFileEntry.failed_oper_notice)
        sendto_gnotice_flags(
            UMODE_ALL, L_ALL, me.name, &me, NULL, "Failed OPER attempt as %s "
                                                  "by %s (%s@%s) - %s",
            name, source_p->name, source_p->username, source_p->host, reason);
}
