 ...
 */
static void
mo_kline(struct Client *client_p, struct Client *source_p, int parc,
         char **parv)
{
    char *reason = NULL;
    char *oper_reason;
    ...
    if((p = strchr(host, '/')) != NULL)
    {
        ...
        {
            sendto_one(source_p,
                       ":%s NOTICE %s :Cannot set K-Lines with CIDR length "
                       "< %d",
    ...
    if(target_server != NULL)
    {
        ...
        cluster_a_line(source_p, "KLINE", CAP_KLN, SHARED_KLINE, "%d %s %s :%s",
...
static void
me_kline(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
...
static void
ms_kline(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    if(parc != 6 || EmptyString(parv[5]))
        return;
 ...
 */
static void
mo_dline(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    ...
    if(parse_aline("DLINE", source_p, parc, parv, AWILD, &dlhost, NULL,
    ...
    if((t = parse_netmask(dlhost, NULL, &bits)) == HM_HOST)
    {
        ...
        irc_getnameinfo((struct sockaddr *)&target_p->ip, target_p->ip.ss_len,
    ...
    if(bits < 8)
    {
        sendto_one(source_p, ":%s NOTICE %s :For safety, bitmasks less than 8 "
                             "require conf access.",
                   me.name, source_p->name);
        return;
    }
...
*/
static void
mo_unkline(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
    char *target_server = NULL;
    char *user, *host;
    ...
    if(parse_aline("UNKLINE", source_p, parc, parv, 0, &user, &host, NULL,
    ...
    if(target_server != NULL)
    {
        ...
        cluster_a_line(source_p, "UNKLINE", CAP_UNKLN, SHARED_UNKLINE, "%s %s",
                       user, host);
    ...
    if(remove_conf_line(KLINE_TYPE, source_p, user, host) > 0)
    {
        ...
        sendto_realops_flags(UMODE_ALL, L_ALL,
                             ...
                             get_oper_name(source_p), user, host);
        ilog(L_NOTICE, "%s removed K-Line for [%s@%s]", source_p->name, user,
             host);
    }
    else
 ...
 */
static void
me_unkline(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
                               ...
                               SHARED_UNKLINE))
    {
        ...
        if(remove_conf_line(KLINE_TYPE, source_p, kuser, khost) > 0)
        {
            ...
            ilog(L_NOTICE, "%s removed K-Line for [%s@%s]", source_p->name,
                 kuser, khost);
        }
        else
            sendto_one(source_p, ":%s NOTICE %s :No K-Line for [%s@%s] found",
...
static void
ms_unkline(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
    if(parc != 4)
        return;
...
*/
static void
mo_undline(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
    const char *cidr;
    ...
    if(remove_conf_line(DLINE_TYPE, source_p, cidr, NULL) > 0)
    {
        ...
        sendto_one(source_p, ":%s NOTICE %s :No D-Line for [%s] found", me.name,
                   source_p->name, cidr);
}
