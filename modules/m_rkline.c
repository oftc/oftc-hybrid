...
struct Message unrkline_msgtab = {"UNRKLINE",
                                  0,
                                  0,
                                  2,
                                  0,
                                  MFLG_SLOW,
                                  0,
                                  {m_unregistered, m_not_oper, ms_unrkline,
                                   me_unrkline, mo_unrkline, m_ignore}};
 ...
 */
static void
mo_rkline(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    ...
    if(parse_aline("RKLINE", source_p, parc, parv, NOUSERLOOKUP, &user, &host,
    ...
    if(target_server != NULL)
    {
        if(HasID(source_p))
        {
            sendto_server(NULL, NULL, CAP_KLN | CAP_TS6, NOCAPS,
                          ":%s RKLINE %s %lu %s %s :%s", source_p->id,
                          target_server, (unsigned long)tkline_time, user, host,
                          reason);
            sendto_server(NULL, NULL, CAP_KLN, CAP_TS6,
                          ":%s RKLINE %s %lu %s %s :%s", source_p->name,
                          target_server, (unsigned long)tkline_time, user, host,
                          reason);
        }
        else
    ...
    apply_rkline(source_p, conf, current_date, cur_time, tkline_time);
}

/* me_rkline - handle remote rkline. no propagation */
static void
me_rkline(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    struct ConfItem *conf    = NULL;
    struct AccessItem *aconf = NULL;
...
static void
ms_rkline(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    if(parc != 6 || EmptyString(parv[5]))
        return;
    ...
    sendto_match_servs(source_p, parv[1], CAP_KLN, "RKLINE %s %s %s %s :%s",
 ...
 */
static int
already_placed_rkline(struct Client *source_p, const char *user,
                      const char *host)
{
    ...
    DLINK_FOREACH(ptr, rkconf_items.head)
    {
        ...
        if(!strcmp(user, aptr->user) && !strcmp(aptr->host, host))
        {
            sendto_one(
                ...
                me.name, source_p->name, user, host, aptr->user, aptr->host,
                aptr->reason ? aptr->reason : "No reason");
            return 1;
        }
    }
 ...
 */
static void
mo_unrkline(struct Client *client_p, struct Client *source_p, int parc,
            char *parv[])
{
    char *target_server = NULL;
    char *user, *host;
    ...
    if(parc < 2 || *parv[1] == '\0')
    {
        sendto_one(source_p, form_str(ERR_NEEDMOREPARAMS), me.name,
                   source_p->name, "UNRKLINE");
        return;
    }

    if(parse_aline("UNRKLINE", source_p, parc, parv, NOUSERLOOKUP, &user, &host,
                   NULL, &target_server, NULL) < 0)
        return;
    ...
    if(remove_conf_line(RKLINE_TYPE, source_p, user, host) > 0)
    {
        ...
        sendto_realops_flags(UMODE_ALL, L_ALL,
                             ...
                             get_oper_name(source_p), user, host);
        ilog(L_NOTICE, "%s removed RK-Line for [%s@%s]", source_p->name, user,
             host);
    }
    else
 ...
 */
static void
me_unrkline(struct Client *client_p, struct Client *source_p, int parc,
            char *parv[])
{
                               ...
                               SHARED_UNKLINE))
    {
        ...
        {
            sendto_one(source_p,
                       ":%s NOTICE %s :RK-Line for [%s@%s] is removed", me.name,
                       source_p->name, user, host);
            sendto_realops_flags(UMODE_ALL, L_ALL,
                                 "%s has removed the RK-Line for: [%s@%s]",
                                 get_oper_name(source_p), user, host);

            ilog(L_NOTICE, "%s removed RK-Line for [%s@%s]", source_p->name,
                 user, host);
        }
        else
            sendto_one(source_p, ":%s NOTICE %s :No RK-Line for [%s@%s] found",
...
static void
ms_unrkline(struct Client *client_p, struct Client *source_p, int parc,
            char *parv[])
{
    if(parc != 4 || EmptyString(parv[3]))
        return;

    sendto_match_servs(source_p, parv[1], CAP_UNKLN, "UNRKLINE %s %s %s",
