...
static void m_message(int, const char *, struct Client *, struct Client *, int,
                      char **);
...
static void msg_client(int p_or_n, const char *command, struct Client *source_p,
...
static void
m_privmsg(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
...
static void
m_notice(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
            ...
            struct Client *source_p, struct Channel *chptr, char *text)
{
    ...
    else
    {
        if(p_or_n != NOTICE)
        {
            ...
            {
                msg_channel_flags(p_or_n, command, client_p, source_p, chptr,
                                  CHFL_CHANOP, text);
            }
        }
    }
...
msg_channel_flags(int p_or_n, const char *command, struct Client *client_p,
                  struct Client *source_p, struct Channel *chptr, int flags,
                  char *text)
{
    int type;
    char c;
    ...
    {
        /*
         * another good catch, lee.  we never would echo to remote clients
         * anyway,
    ...
    if(MyClient(target_p))
    {
        if(!IsServer(source_p) && IsSetCallerId(target_p))
        {
            ...
            if(source_p == target_p || accept_message(source_p, target_p) ||
               (IsOper(source_p) &&
                (ConfigFileEntry.opers_bypass_callerid == 1)))
            {
                sendto_one(target_p, ":%s!%s@%s %s %s :%s", source_p->name,
                           source_p->username, source_p->host, command,
                           target_p->name, text);
            }
            else
            {
                    ...
                    ConfigFileEntry.caller_id_wait) < CurrentTime)
                {
                    ...
                    sendto_one(target_p, form_str(RPL_UMODEGMSG), me.name,
                               target_p->name,
       ...
       IsClient(source_p) && !IsConfCanFlood(source_p))
    {
           ...
           target_p->localClient->flood_noticed)
        {
            ...
            if(MyClient(source_p) && (p_or_n != NOTICE))
                sendto_one(source_p, ":%s NOTICE %s :*** Message to %s "
                                     "throttled due to flooding",
               ...
               struct Client *source_p, char *nick, char *text)
{
        ...
        if((target_p = find_server(server + 1)) != NULL)
        {
            if(!IsMe(target_p))
            {
                ...
                sendto_one(target_p, ":%s %s %s :%s",
                           ID_or_name(source_p, target_p->from), command, nick,
                           text);
    ...
    if(*nick == '$')
    {
        ...
        {
            sendto_one(source_p, ":%s NOTICE %s :The command %s %s is no "
                                 "longer supported, please use $%s",
        ...
        {
            sendto_one(source_p, form_str(ERR_NOTOPLEVEL), me.name,
                       source_p->name, nick);
            return;
        }
