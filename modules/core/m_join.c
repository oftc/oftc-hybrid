...
struct Message join_msgtab = {
    ...
    0,      2,
    0,      MFLG_SLOW,
    0,      {m_unregistered, m_join, ms_join, m_ignore, m_join, m_ignore}};
        ...
        chan = strtoken(&p, NULL, ","))
    {
           ...
           ((!hash_find_resv(chan)) == ConfigChannel.restrict_channels))
        {
            ...
            sendto_gnotice_flags(UMODE_SPY, L_ALL, me.name, &me, NULL,
                                 "User %s (%s@%s) is attempting to join "
                                 "locally juped channel %s",
        ...
        else
        {
            ...
            if(MyClient(source_p))
                sendto_gnotice_flags(UMODE_SPY, L_ALL, me.name, &me, NULL,
                                     "Channel %s created by %s!%s@%s", chan,
                                     source_p->name, source_p->username,
                                     source_p->host);
        }
        ...
        {
            sendto_server(client_p, chptr, CAP_TS6, NOCAPS, ":%s JOIN %lu %s +",
 ...
 */
static void
ms_join(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
    time_t newts           = 0;
    time_t oldts           = 0;
    ...
    sendto_server(client_p, chptr, CAP_TS6, NOCAPS, NOFLAGS,
                  ":%s JOIN %lu %s +", ID(source_p),
                  (unsigned long)chptr->channelts, chptr->chname);
    sendto_server(client_p, chptr, NOCAPS, CAP_TS6, NOFLAGS,
                  ":%s SJOIN %lu %s + :%s", source_p->servptr->name,
                  (unsigned long)chptr->channelts, chptr->chname,
                  source_p->name);
}
...
do_join_0(struct Client *client_p, struct Client *source_p)
{
    ...
    DLINK_FOREACH_SAFE(ptr, ptr_next, source_p->channel.head)
    {
        ...
        sendto_server(client_p, chptr, CAP_TS6, NOCAPS, NOFLAGS, ":%s PART %s",
                      ID(source_p), chptr->chname);
        sendto_server(client_p, chptr, NOCAPS, CAP_TS6, NOFLAGS, ":%s PART %s",
                      source_p->name, chptr->chname);
