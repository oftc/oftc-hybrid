...
static void remove_ban_list(struct Channel *, struct Client *, dlink_list *,
                            char, int);
 ...
 */
static void
ms_sjoin(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    ...
    char uid_buf
        [IRCD_BUFSIZE]; /* buffer for modes/prefixes for CAP_TS6 servers */
    char *nick_ptr,
        *uid_ptr; /* pointers used for making the two mode/prefix buffers */
    char *p;      /* pointer used making sjbuf */
    dlink_node *m;
    const char *servername =
        (ConfigServerHide.hide_servers || IsHidden(source_p)) ? me.name
                                                              : source_p->name;
    ...
    if((chptr = hash_find_channel(parv[2])) == NULL)
    {
        ...
        if(MyClient(source_p))
            sendto_gnotice_flags(UMODE_SPY, L_ALL, me.name, &me, NULL,
                                 "Channel %s created by %s!%s@%s", parv[2],
    ...
    buflen =
        ircsprintf(nick_buf, ":%s SJOIN %lu %s %s %s:", source_p->name,
                   (unsigned long)tstosend, chptr->chname, modebuf, parabuf);
    nick_ptr = nick_buf + buflen;

    buflen =
        ircsprintf(uid_buf, ":%s SJOIN %lu %s %s %s:", ID(source_p),
                   (unsigned long)tstosend, chptr->chname, modebuf, parabuf);
    ...
    *mbuf++ = '+';
    ...
    while(*s)
    {
         ...
         * sometimes.
         */
        if(target_p == NULL || target_p->from != client_p ||
           !IsClient(target_p))
        {
            goto nextnick;
        }
            ...
            *mbuf++       = 'o';
            ...
            if(pargs >= MAXMODEPARAMS)
            {
                ...
                *mbuf = '\0';
                ...
                sendto_channel_local(ALL_MEMBERS, NO, chptr, ":%s MODE %s %s%s",
                                     servername, chptr->chname, modebuf,
                                     sendbuf);
                mbuf    = modebuf;
                *mbuf++ = '+';
    ...
    *(uid_ptr - 1)  = '\0';
    ...
    if(HasID(source_p) && !keep_our_modes)
    {
        ...
        if(dlink_list_length(&chptr->quietlist) > 0)
            remove_ban_list(chptr, client_p, &chptr->quietlist, 'q', CAP_QUIET);
 ...
 */
static void
remove_a_mode(struct Channel *chptr, struct Client *source_p, int mask,
              char flag)
{
    dlink_node *ptr;
    ...
    DLINK_FOREACH(ptr, chptr->members.head)
    {
        ...
        *mbuf++ = flag;
        ...
        if(count >= MAXMODEPARAMS)
        {
            ...
            *mbuf = '\0';
            sendto_channel_local(
                ALL_MEMBERS, NO, chptr, ":%s MODE %s %s%s",
 ...
 *		  non-ts6 servers linked through another uplink other than
 *source_p
 */
static void
                ...
                dlink_list *list, char c, int cap)
{
    ...
    cur_len = mlen =
        ircsprintf(lmodebuf, ":%s MODE %s -", source_p->name, chptr->chname);
    mbuf = lmodebuf + mlen;
    ...
    DLINK_FOREACH_SAFE(ptr, next_ptr, list->head)
    {
           ...
           (cur_len + 1 /* space between */ + (plen - 1)) > IRCD_BUFSIZE - 2)
        {
            ...
            *mbuf = *(pbuf - 1) = '\0';
            sendto_channel_local(ALL_MEMBERS, NO, chptr, "%s %s", lmodebuf,
                                 lparabuf);
            sendto_server(source_p, chptr, cap, CAP_TS6, NOFLAGS, "%s %s",
                          lmodebuf, lparabuf);
    ...
    *mbuf = *(pbuf - 1) = '\0';
    ...
    sendto_server(source_p, chptr, cap, CAP_TS6, NOFLAGS, "%s %s", lmodebuf,
                  lparabuf);
}
