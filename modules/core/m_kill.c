...
struct Message kill_msgtab = {
    ...
    0,      2,
    0,      MFLG_SLOW,
    0,      {m_unregistered, m_not_oper, ms_kill, m_ignore, mo_kill, m_ignore}};
 ...
 */
static void
mo_kill(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
    ...
    {
        sendto_one(source_p, form_str(ERR_CANTKILLSERVER), me.name,
                   source_p->name);
        return;
    }
    ...
    if(MyConnect(target_p))
        sendto_one(target_p, ":%s!%s@%s KILL %s :%s", source_p->name,
                   source_p->username, source_p->host, target_p->name, reason);

    /* Do not change the format of this message.  There's no point in changing
     * messages
    ...
    ilog(L_INFO, "KILL From %s For %s Path %s (%s)", source_p->name,
         target_p->name, me.name, reason);
 ...
 */
static void
ms_kill(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
    struct Client *target_p;
    char *user;
        ...
        if(IsServer(source_p))
        {
            ...
            if((IsHidden(source_p) || ConfigServerHide.hide_servers) &&
               !IsOper(target_p))
                ...
                sendto_one(target_p, ":%s KILL %s :%s", source_p->name,
...
static void
relay_kill(struct Client *one, struct Client *source_p, struct Client *target_p,
           const char *inpath, const char *reason)
{
    ...
    DLINK_FOREACH(ptr, serv_list.head)
    {
        ...
        {
            sendto_one(client_p, ":%s KILL %s :%s!%s!%s!%s (%s)", from, to,
                       me.name, source_p->host, source_p->username,
                       source_p->name, reason);
        }
        else
        {
            sendto_one(client_p, ":%s KILL %s :%s %s", from, to, inpath,
                       reason);
        }
    }
}
