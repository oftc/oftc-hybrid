...
struct Message kick_msgtab = {
    ...
    0,      3,
    0,      MFLG_SLOW,
    0,      {m_unregistered, m_kick, m_kick, m_ignore, m_kick, m_ignore}};
...
m_kick(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
    ...
    if((ms_target = find_channel_link(who, chptr)) != NULL)
    {
        if(IsGod(who))
        {
            ...
            ircsprintf(tmp,
                       "%s is using God mode: to evade KICK from %s: %s %s %s",
                       who->name, source_p->name, chptr->chname, parv[2],
                       parv[3] ? parv[3] : "");
        ...
        sendto_one(source_p, form_str(ERR_USERNOTINCHANNEL), from, to, user,
                   name);
}
