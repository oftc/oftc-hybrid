...
struct Message sid_msgtab = {
    ...
    0,     5,
    0,     MFLG_SLOW,
    0,     {m_error, m_ignore, ms_sid, m_ignore, m_ignore, m_ignore}};
 ...
 */
static void
mr_server(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    char info[REALLEN + 1];
    char *name;
    ...
    case -4:
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             "Invalid servername %s from %s", name,
                             get_client_name(client_p, SHOW_IP));
    ...
    if((me.id[0] && client_p->id[0] &&
        (target_p = hash_find_id(client_p->id))) ||
 ...
 */
static void
ms_server(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    char info[REALLEN + 1];
    char *name;
    ...
    if(strchr(name, '.') == NULL)
    {
        ...
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             ...
                             /* inpath */ get_client_name(client_p, SHOW_IP),
                             name);
    ...
    {
        sendto_realops_flags(UMODE_ALL, L_OPER, "already linked %s at %s:%d",
                             target_p->name, __FILE__, __LINE__);
...
#define SID_GECOS parv[4]
     ...
     */
    if(strchr(SID_NAME, '.') ==
           NULL || /* servernames must have a '.' and nicks must not */
       strlen(SID_NAME) >
           HOSTLEN) /* ensure the name of the server is not too long  */
    {
        /* Server trying to use the same name as a person. Would
         * cause a fair bit of confusion. Enough to make it hellish
         * for a while and servers to send stuff to the wrong place.
    ...
    if((target_p->node.prev != NULL) || (target_p->node.next != NULL))
    {
        sendto_realops_flags(UMODE_ALL, L_ADMIN, "already linked %s at %s:%d",
                             target_p->name, __FILE__, __LINE__);
        ilog(L_ERROR, "already linked %s at %s:%d", target_p->name, __FILE__,
             __LINE__);
        assert(0 == 3);
    }
    else
    ...
    DLINK_FOREACH_SAFE(ptr, ptr_next, serv_list.head)
    {
        ...
        if(IsCapable(bclient_p, CAP_TS6))
            ...
            sendto_one(bclient_p, ":%s SERVER %s %d :%s%s", source_p->name,
                       target_p->name, hop + 1,
                       IsHidden(target_p) ? "(H) " : "", target_p->info);
    }
}
