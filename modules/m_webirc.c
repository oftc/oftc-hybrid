...
struct Message webirc_msgtab = {
    ...
    0,        5,
    0,        MFLG_SLOW,
    0,        {mr_webirc, m_ignore, m_ignore, m_ignore, m_ignore, m_ignore}};
 ...
 *      parv[2] = sockhost / ip
 *      parv[3] = realhost
 *      parv[4] = cloak / same as realhost if no cloak
 *      parv[5] = certfp
 */
static void
          ...
          char *parv[])
{
    ...
    aconf = find_address_conf(
        source_p->host, IsGotId(source_p) ? source_p->username : "webirc",
        &source_p->ip, source_p->aftype, parv[1], source_p->certfp);
    ...
    if(strncmp(parv[3], parv[4], sizeof(source_p->realhost)) != 0)
    {
        if(valid_hostname(parv[3]) == 0)
            strlcpy(source_p->realhost, source_p->sockhost,
                    sizeof(source_p->realhost));
        else
    ...
    if(parc == 6)
        strlcpy(source_p->certfp, parv[5], sizeof(source_p->certfp));
    ...
    sendto_gnotice_flags(UMODE_CCONN, L_ALL, me.name, &me, NULL,
                         "CGI:IRC host/IP set %s to %s", original_sockhost,
                         host);
}
