...
struct Message list_msgtab = {
    ...
    0,      0,
    0,      MFLG_SLOW,
    0,      {m_unregistered, m_list, m_ignore, m_ignore, mo_list, m_ignore}};
...
do_list(struct Client *source_p, int parc, char *parv[])
{
    ...
    if(MyConnect(source_p))
    {
        if(source_p->localClient->list_task != NULL)
        {
            ...
            sendto_one(source_p, form_str(RPL_LISTEND), me.name,
                       source_p->name);
            return;
        }
    }
    ...
    sendto_one(source_p, form_str(RPL_LISTSTART),
               MyConnect(source_p) ? me.name : ID(&me),
               MyConnect(source_p) ? source_p->name : ID(source_p));
    safe_list_channels(source_p, lt,
                       no_masked_channels && lt->show_mask.head != NULL,
                       !MyConnect(source_p));
}
...
*/
static void
mo_list(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
    do_list(source_p, parc, parv);
}
