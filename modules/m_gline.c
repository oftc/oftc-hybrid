...
/* internal functions */
static void set_local_gline(const struct Client *, const char *, const char *,
                            const char *);

#ifdef GLINE_VOTING
static int check_majority_gline(const struct Client *, const char *,
                                const char *, const char *);

static void add_new_majority_gline(const struct Client *, const char *,
                                   const char *, const char *);
#endif /* GLINE_VOTING */
...
static void
mo_gline(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    char *user   = NULL;
    ...
    {
        sendto_one(source_p, ":%s NOTICE %s :GLINE disabled", me.name,
                   source_p->name);
        return;
    }
    ...
    if(parse_aline("GLINE", source_p, parc, parv, AWILD, &user, &host, NULL,
                   NULL, &reason) < 0)
        return;
    ...
    ilog(L_TRACE, "#gline for %s@%s [%s] requested by %s!%s@%s", user, host,
    ...
    sendto_server(NULL, NULL, CAP_GLN | CAP_TS6, NOCAPS, ":%s GLINE %s %s :%s",
                  ID(source_p), user, host, reason);
    sendto_server(NULL, NULL, CAP_GLN, CAP_TS6, ":%s GLINE %s %s :%s",
                  source_p->name, user, host, reason);

    /* 8 param for hyb-6 */
    sendto_server(NULL, NULL, CAP_TS6, CAP_GLN, NOFLAGS,
                  ":%s GLINE %s %s %s %s %s %s :%s", ID(&me), ID(source_p),
                  source_p->username, source_p->host, source_p->servptr->name,
                  user, host, reason);
    sendto_server(NULL, NULL, NOCAPS, CAP_GLN | CAP_TS6, NOFLAGS,
                  ":%s GLINE %s %s %s %s %s %s :%s", me.name, source_p->name,
                  source_p->username, source_p->host, source_p->servptr->name,
                  user, host, reason);
}
...
static void
ms_gline(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
...
static void
me_gline(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
...
static void
do_sgline(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[], int prop)
{
    ...
    {
        sendto_server(client_p, NULL, CAP_GLN, NOCAPS, ":%s GLINE %s %s :%s",
                      source_p->name, user, host, reason);
        /* hyb-6 version to the rest */
        sendto_server(client_p, NULL, NOCAPS, CAP_GLN, NOFLAGS,
    ...
    if(ConfigFileEntry.glines)
    {
            ...
            if((p = strchr(host, '/')))
            {
                ...
                if(bitlen < min_bitlen)
                {
                    sendto_realops_flags(
                        ...
                        source_p->servptr->name, min_bitlen, user, host,
                        reason);
                    return;
                }
            }
        ...
        sendto_realops_flags(UMODE_ALL, L_ALL,
                             "%s requesting G-Line for [%s@%s] [%s]",
                             get_oper_name(source_p), user, host, reason);
        ilog(L_TRACE, "#gline for %s@%s [%s] requested by %s", user, host,
             reason, get_oper_name(source_p));
#else
        set_local_gline(source_p, user, host, reason);
#endif /* GLINE_VOTING */
    }
                ...
                const char *host, const char *reason)
{
    ...
    sendto_realops_flags(UMODE_ALL, L_ALL, "%s added G-Line for [%s@%s] [%s]",
                         get_oper_name(source_p), aconf->user, aconf->host,
                         aconf->reason);
    ilog(L_TRACE, "%s added G-Line for [%s@%s] [%s]", get_oper_name(source_p),
 ...
 */
static void
add_new_majority_gline(const struct Client *source_p, const char *user,
                       const char *host, const char *reason)
{
    ...
    strlcpy(pending->oper_server1, source_p->servptr->name,
            sizeof(pending->oper_server1));
 ...
 */
static int
check_majority_gline(const struct Client *source_p, const char *user,
                     const char *host, const char *reason)
{
    ...
    DLINK_FOREACH(pending_node, pending_glines.head)
    {
        ...
        {
            if(((irccmp(gline_pending_ptr->oper_user1, source_p->username) ==
                 0) ||
                (irccmp(gline_pending_ptr->oper_host1, source_p->host) == 0)) ||
               (irccmp(gline_pending_ptr->oper_server1,
                       source_p->servptr->name) == 0))
            {
            ...
            if(gline_pending_ptr->oper_user2[0] != '\0')
            {
                ...
                if(((irccmp(gline_pending_ptr->oper_user2,
                            source_p->username) == 0) ||
                    (irccmp(gline_pending_ptr->oper_host2, source_p->host) ==
                     0)) ||
                   (irccmp(gline_pending_ptr->oper_server2,
                           source_p->servptr->name) == 0))
                {
                    return (GLINE_ALREADY_VOTED);
                }
            ...
            else
            {
                ...
                strlcpy(gline_pending_ptr->oper_server2,
 ...
 */
static void
mo_ungline(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
    char *user, *host;
    ...
    if(!ConfigFileEntry.glines)
    {
        sendto_one(source_p, ":%s NOTICE %s :UNGLINE disabled", me.name,
                   source_p->name);
        return;
    }
    ...
    if(parse_aline("UNGLINE", source_p, parc, parv, 0, &user, &host, NULL, NULL,
                   NULL) < 0)
        return;
    ...
    if(remove_gline_match(user, host))
    {
        ...
        ilog(L_NOTICE, "%s removed G-Line for [%s@%s]", get_oper_name(source_p),
             user, host);
    }
    else
    {
        sendto_one(source_p, ":%s NOTICE %s :No G-Line for %s@%s", me.name,
