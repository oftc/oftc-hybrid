...
struct Message cap_msgtab = {
    "CAP", 0,         0, 2,
    0,     MFLG_SLOW, 0, {m_cap, m_cap, m_ignore, m_ignore, m_cap, m_ignore}};
...
find_cap(const char **caplist_p, int *neg_p)
{
    ...
    *neg_p = 0; /* clear negative flag... */
    ...
    if(*caplist)
    {
        ...
        {
            /* Couldn't find the capability; advance to first whitespace
             * character */
            while(*caplist && !IsSpace(*caplist))
                ++caplist;
        }
 ...
 */
static int
send_caplist(struct Client *sptr, unsigned int set, unsigned int rem,
             const char *subcmd)
{
    ...
    for(i = 0, loc = 0; i < CAPAB_LIST_LEN; ++i)
    {
        ...
        loc += snprintf(capbuf + loc, sizeof(capbuf) - loc, "%s%s", pfx,
                        capab_list[i].name);
    }
...
cap_clear(struct Client *sptr, const char *caplist)
{
    ...
    for(ii = 0; ii < CAPAB_LIST_LEN; ++ii)
    {
        ...
        if(!(sptr->localClient->cap_active & cap->cap) ||
...
} cmdlist[] = {{"ACK", cap_ack},   {"CLEAR", cap_clear}, {"END", cap_end},
               {"LIST", cap_list}, {"LS", cap_ls},       {"NAK", NULL},
               {"REQ", cap_req}};
...
m_cap(struct Client *cptr, struct Client *sptr, int parc, char *parv[])
{
    ...
    if(!(cmd = bsearch(subcmd, cmdlist, sizeof(cmdlist) / sizeof(struct subcmd),
                       sizeof(struct subcmd), (bqcmp)subcmd_search)))
    {
        sendto_one(sptr, form_str(ERR_INVALIDCAPCMD), me.name, sptr->name,
                   subcmd);
        return;
    }
