...
struct Message away_msgtab = {
    ...
    0,      0,
    0,      MFLG_SLOW,
    0,      {m_unregistered, m_away, ms_away, m_ignore, mo_away, m_ignore}};
...
m_away(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
    ...
    if(parc < 2 || EmptyString(parv[1]))
    {
        ...
        if(cur_away_msg)
        {
            ...
            sendto_server(client_p, NULL, CAP_TS6, NOCAPS, NOFLAGS, ":%s AWAY",
                          ID(source_p));
            sendto_server(client_p, NULL, NOCAPS, CAP_TS6, NOFLAGS, ":%s AWAY",
                          source_p->name);
    ...
    if(!cur_away_msg)
    {
        sendto_server(client_p, NULL, CAP_TS6, NOCAPS, NOFLAGS, ":%s AWAY :%s",
                      ID(source_p), new_away_msg);
        sendto_server(client_p, NULL, NOCAPS, CAP_TS6, NOFLAGS, ":%s AWAY :%s",
                      source_p->name, new_away_msg);
    }
    else
        MyFree(cur_away_msg);
...
static void
mo_away(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
    char *cur_away_msg = source_p->away;
    char *new_away_msg = NULL;
...
static void
ms_away(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
