...
*/
static void
m_whowas(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
...
static void
mo_whowas(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    if(parc < 2 || *parv[1] == '\0')
    {
...
static void
whowas_do(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    ...
    if(parc > 3)
        if(hunt_server(client_p, source_p, ":%s WHOWAS %s %s :%s", 3, parc,
                       parv) != HUNTED_ISME)
            return;
    ...
    for(; temp; temp = temp->next)
    {
        ...
        {
            sendto_one(source_p, form_str(RPL_WHOWASUSER), me.name,
                       source_p->name, temp->name, temp->username,
            ...
            if(ConfigServerHide.hide_servers && !IsOper(source_p))
                sendto_one(source_p, form_str(RPL_WHOISSERVER), me.name,
                           source_p->name, temp->name, ServerInfo.network_name,
                           myctime(temp->logoff));
            else
                sendto_one(source_p, form_str(RPL_WHOISSERVER), me.name,
                           source_p->name, temp->name, temp->servername,
                           myctime(temp->logoff));
            cur++;
        }
    ...
    if(!cur)
        sendto_one(source_p, form_str(ERR_WASNOSUCHNICK), me.name,
                   source_p->name, nick);

    sendto_one(source_p, form_str(RPL_ENDOFWHOWAS), me.name, source_p->name,
               parv[1]);
}
