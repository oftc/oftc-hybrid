...
already_placed_rxline(struct Client *source_p, const char *gecos)
{
    ...
    DLINK_FOREACH(ptr, rxconf_items.head)
    {
        ...
        {
            sendto_one(
                source_p, ":%s NOTICE %s :[%s] already RX-Lined by [%s] - %s",
 ...
 */
static void
mo_rxline(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    char *reason        = NULL;
    char *gecos         = NULL;
    ...
    if(parse_aline("RXLINE", source_p, parc, parv, AWILD, &gecos, NULL,
    ...
    if(target_server != NULL)
    {
        sendto_match_servs(source_p, target_server, CAP_CLUSTER,
                           "RXLINE %s %s %d :%s", target_server, gecos,
 ...
 */
static void
ms_rxline(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    int t_sec = 0;
    ...
    sendto_match_servs(source_p, parv[1], CAP_CLUSTER, "RXLINE %s %s %s :%s",
...
static void
mo_unrxline(struct Client *client_p, struct Client *source_p, int parc,
            char *parv[])
{
    char *gecos         = NULL;
    char *target_server = NULL;
    ...
    if(parse_aline("UNRXLINE", source_p, parc, parv, 0, &gecos, NULL, NULL,
 ...
 */
static void
ms_unrxline(struct Client *client_p, struct Client *source_p, int parc,
            char *parv[])
{
    if(parc != 3 || EmptyString(parv[2]))
        return;

    sendto_match_servs(source_p, parv[1], CAP_CLUSTER, "UNRXLINE %s %s",
                       parv[1], parv[2]);
...
remove_xline(struct Client *source_p, char *gecos)
{
    ...
    if(remove_txline(gecos))
    {
        ...
        ilog(L_NOTICE, "%s removed temporary RX-Line for [%s]", source_p->name,
             gecos);
        return;
    }
    ...
    if(remove_conf_line(RXLINE_TYPE, source_p, gecos, NULL) > 0)
    {
        ...
        sendto_one(source_p, ":%s NOTICE %s :No RX-Line for %s", me.name,
                   source_p->name, gecos);
}
