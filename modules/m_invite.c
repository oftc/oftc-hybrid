...
*/
static void
m_invite(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    ...
    if(!MyConnect(target_p) && (*parv[2] == '&'))
    {
        if(ConfigServerHide.hide_servers == 0)
            sendto_one(source_p, form_str(ERR_USERNOTONSERV), me.name,
    ...
    if((chptr->mode.mode & (MODE_INVITEONLY | MODE_PRIVATE)))
    {
        ...
        if(MyConnect(source_p) && !chop)
            sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                                 "%s is using God mode: INVITE %s %s",
    ...
    {
        sendto_one(source_p, form_str(RPL_INVITING), me.name, source_p->name,
    ...
    if(MyConnect(target_p))
    {
        ...
        if(chptr->mode.mode & MODE_INVITEONLY)
        {
            if(chptr->mode.mode & MODE_PRIVATE)
            {
                ...
                sendto_channel_remote(
                    source_p, client_p, CHFL_CHANOP | CHFL_HALFOP, NOCAPS,
                    NOCAPS, chptr, ":%s NOTICE %s :%s is inviting %s to %s.",
        ...
        sendto_one(target_p, ":%s INVITE %s %s %lu",
                   ...
                   ID_or_name(target_p, target_p->from), chptr->chname,
