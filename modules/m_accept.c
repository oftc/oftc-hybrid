 ...
 */
static void
m_accept(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    char *nick;
    char *p                   = NULL;
        ...
        nick = strtoken(&p, NULL, ","))
    {
        ...
        {
            sendto_one(source_p, form_str(ERR_ACCEPTNOT), me.name,
        ...
        nick = strtoken(&p, NULL, ","), accept_num++)
    {
        ...
        if(accept_message(target_p, source_p))
        {
            sendto_one(source_p, form_str(ERR_ACCEPTEXIST), me.name,
                       source_p->name, target_p->name);
            continue;
        }
        ...
        {
            sendto_one(source_p, form_str(ERR_ACCEPTFULL), me.name,
                       source_p->name);
            return;
        }
...
list_accepts(struct Client *source_p)
{
    ...
    DLINK_FOREACH(ptr, source_p->allow_list.head)
    {
        ...
        {
            *(t - 1) = '\0';
            sendto_one(source_p, form_str(RPL_ACCEPTLIST), me.name,
                       source_p->name, nicks);
            t = nicks;
        }
    ...
    {
        *(t - 1) = '\0';
        sendto_one(source_p, form_str(RPL_ACCEPTLIST), me.name, source_p->name,
                   nicks);
    }
