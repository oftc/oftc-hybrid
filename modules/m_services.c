...
#define services_function(a, b, c)                                             \
    static void a(struct Client *client_p, struct Client *source_p, int parc,  \
                  char *parv[])                                                \
    {                                                                          \
 ...
 */
static void
m_svsnick(struct Client *client_p, struct Client *source_p, int parc,
          char *parv[])
{
    ...
    {
        sendto_one(source_p, form_str(ERR_NOSUCHNICK), me.name, parv[0],
                   parv[1]);
        return;
    }
    ...
    if(MyConnect(target_p))
    {
        ...
        sendto_one(target_p, ":%s ENCAP %s SVSNICK %s %s", me.name,
                   target_p->servptr->name, ID(target_p), newnick);
}
                ...
                services_function(m_hostserv, "HostServ", "HOSTSERV")
                    services_function(m_memoserv, "MemoServ", "MEMOSERV")
                        services_function(m_nickserv, "NickServ", "NICKSERV")
                            services_function(m_operserv, "OperServ",
                                              "OPERSERV")
                                services_function(m_seenserv, "SeenServ",
                                                  "SEENSERV")
                                    services_function(m_statserv, "StatServ",
                                                      "STATSERV")
 ...
 */
static void
m_identify(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
    struct Client *target_p = NULL;
...
deliver_services_msg(const char *service, const char *command,
                     struct Client *client_p, struct Client *source_p, int parc,
                     char *parv[])
{
    struct Client *target_p = NULL;
    ...
    else
    {
        ...
        sendto_one(target_p, ":%s PRIVMSG %s@%s :%s", source_p->name, service,
                   SERVICES_NAME, buf);
    }
}
