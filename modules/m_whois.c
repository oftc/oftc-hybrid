...
struct Message whois_msgtab = {
    ...
    0,       0,
    0,       MFLG_SLOW,
    0,       {m_unregistered, m_whois, mo_whois, m_ignore, mo_whois, m_ignore}};
...
*/
static void
m_whois(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
...
*/
static void
mo_whois(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    if(parc < 2 || EmptyString(parv[1]))
    {
...
do_whois(struct Client *source_p, int parc, char **parv)
{
    ...
    sendto_one(source_p, form_str(RPL_ENDOFWHOIS), me.name, source_p->name,
               parv[1]);
}
...
whois_person(struct Client *source_p, struct Client *target_p)
{
    ...
    sendto_one(source_p, form_str(RPL_WHOISUSER), me.name, source_p->name,
               target_p->name, target_p->username, target_p->host,
               target_p->info);

    if(!IsService(target_p))
    {
        cur_len = mlen = ircsprintf(buf, form_str(RPL_WHOISCHANNELS), me.name,
        ...
        DLINK_FOREACH(lp, target_p->channel.head)
        {
            ...
            if(ShowChannel(source_p, chptr) || IsGod(source_p))
            {
                ...
                if((cur_len + 3 + strlen(chptr->chname) + 1) >
                   (IRCD_BUFSIZE - 2))
                {
                    *(t - 1) = '\0';
                ...
                if(!ShowChannel(source_p, chptr))
                    tlen = ircsprintf(t, "%s%%%s ", get_member_status(ms, YES),
                                      chptr->chname);
                else
                    tlen = ircsprintf(t, "%s%s ", get_member_status(ms, YES),
                                      chptr->chname);
    ...
    if(IsOper(source_p) || !ConfigServerHide.hide_servers ||
       target_p == source_p)
        ...
        sendto_one(source_p, form_str(RPL_WHOISSERVER), me.name, source_p->name,
                   target_p->name, ConfigServerHide.hidden_name,
                   ServerInfo.network_desc);

    if(target_p->away != NULL)
        sendto_one(source_p, form_str(RPL_AWAY), me.name, source_p->name,
                   target_p->name, target_p->away);
    ...
    if(IsSetCallerId(target_p) && !IsSoftCallerId(target_p))
        sendto_one(source_p, form_str(RPL_TARGUMODEG), me.name, source_p->name,
                   target_p->name);

    if(IsOper(target_p) && !IsService(target_p))
        sendto_one(source_p,
                   form_str((IsAdmin(target_p) && !IsOperHiddenAdmin(target_p))
                                ? RPL_WHOISADMIN
                                : RPL_WHOISOPERATOR),
                   me.name, source_p->name, target_p->name);

    if(IsOper(source_p) && IsCaptured(target_p))
        sendto_one(source_p, form_str(RPL_ISCAPTURED), me.name, source_p->name,
                   target_p->name);
    ...
    if(IsService(target_p))
        sendto_one(source_p, form_str(RPL_WHOISSERVICE), me.name,
                   source_p->name, target_p->name);
    ...
    if(MyConnect(target_p)) /* Can't do any of this if not local! db */
    {
        ...
        {
            sendto_one(source_p, form_str(RPL_WHOISSSL), me.name,
                       source_p->name, target_p->name);
...
#endif
        sendto_one(source_p, form_str(RPL_WHOISIDLE), me.name, source_p->name,
                   target_p->name, CurrentTime - target_p->localClient->last,
                   target_p->firsttime);
    }
}
