...
struct Message stats_msgtab = {
    ...
    0,       2,
    0,       MFLG_SLOW,
    0,       {m_unregistered, m_stats, ms_stats, m_ignore, mo_stats, m_ignore}};
...
static void stats_L_list(struct Client *s, char *, int, int, dlink_list *,
                         char);
 ...
 */
static void
m_stats(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
 ...
 */
static void
mo_stats(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    if(hunt_server(client_p, source_p, ":%s STATS %s :%s", 2, parc, parv) !=
       HUNTED_ISME)
...
#define hzz HZ
    ...
    {
        sendto_one(source_p, ":%s NOTICE %s :Getruseage error: %s", me.name,
                   source_p->name, strerror(errno));
        return;
    }
    ...
    sendto_one(source_p, ":%s %d %s R :CPU Secs %d:%d User %d:%d System %d:%d",
    ...
    sendto_one(source_p, ":%s %d %s R :RSS %ld ShMem %ld Data %ld Stack %ld",
               ...
               (rus.ru_isrss / rup));
    sendto_one(source_p, ":%s %d %s R :Swaps %d Reclaims %d Faults %d", me.name,
...
count_memory(struct Client *source_p)
{
    ...
    DLINK_FOREACH(gptr, global_client_list.head)
    {
        ...
        if(MyConnect(target_p))
        {
            ...
            users_invited_count +=
                dlink_list_length(&target_p->localClient->invited);
        }
        else
            ++remote_client_count;
    ...
    channel_memory =
        dlink_list_length(&global_channel_list) * sizeof(struct Channel);
  ...
  DLINK_FOREACH(dlink, ConfigItemList.head)
  {
    ...
    sendto_one(source_p, ":%s %d %s z :User aways %u(%d)", me.name,
               RPL_STATSDEBUG, source_p->name, aways_counted, (int)away_memory);

    sendto_one(source_p, ":%s %d %s z :Attached confs %u(%lu)", me.name,
               RPL_STATSDEBUG, source_p->name, local_client_conf_count,
...
/* XXX  ConfigItemList fix */
  ...
  sendto_one(source_p, ":%s %d %s z :Conflines %lu(%d)",
    ...
    sendto_one(
        source_p, ":%s %d %s z :Resv channels %lu(%lu) nicks %lu(%lu)", me.name,
        RPL_STATSDEBUG, source_p->name, dlink_list_length(&resv_channel_list),
    ...
    sendto_one(source_p, ":%s %d %s z :Classes %u(%lu)", me.name,
               RPL_STATSDEBUG, source_p->name, class_count,
               (unsigned long)(class_count * sizeof(struct ClassItem)));

    sendto_one(source_p, ":%s %d %s z :Channels %lu(%lu) Topics %u(%d)",
               me.name, RPL_STATSDEBUG, source_p->name,
               dlink_list_length(&global_channel_list), channel_memory,
               topic_count, topic_count * (TOPICLEN + 1 + USERHOST_REPLYLEN));

    sendto_one(source_p, ":%s %d %s z :Bans %u(%u)", me.name, RPL_STATSDEBUG,
               source_p->name, channel_bans, channel_ban_memory);

    sendto_one(source_p, ":%s %d %s z :Exceptions %u(%u)", me.name,
               RPL_STATSDEBUG, source_p->name, channel_except,
               channel_except_memory);

    sendto_one(source_p, ":%s %d %s z :Invex %u(%u)", me.name, RPL_STATSDEBUG,
               source_p->name, channel_invex, channel_invex_memory);
    ...
    sendto_one(source_p, ":%s %d %s z :Safelist %u(%u)", me.name,
               RPL_STATSDEBUG, source_p->name, safelist_count, safelist_memory);

    sendto_one(source_p, ":%s %d %s z :Whowas users %u(%lu)", me.name,
               RPL_STATSDEBUG, source_p->name, wwu,
               (unsigned long)(wwu * sizeof(struct Client)));

    sendto_one(source_p, ":%s %d %s z :Whowas array %u(%d)", me.name,
               RPL_STATSDEBUG, source_p->name, NICKNAMEHISTORYLENGTH, (int)wwm);
    ...
    ****/
    ...
    sendto_one(source_p, ":%s %d %s z :iphash %u(%d)", me.name, RPL_STATSDEBUG,
               source_p->name, number_ips_stored, (int)mem_ips_stored);

    total_memory = totww + total_channel_memory + conf_memory +
                   class_count * sizeof(struct ClassItem);
    ...
    sendto_one(source_p,
               ...
               me.name, RPL_STATSDEBUG, source_p->name, (int)total_memory,
               get_maxrss());

    if(getrlimit(RLIMIT_FD_MAX, &rlim) == 0)
    {
        sendto_one(source_p, ":%s %d %s z :rlimit_nofile: soft: %d; hard: %d",
                   me.name, RPL_STATSDEBUG, source_p->name, (int)rlim.rlim_cur,
                   (int)rlim.rlim_max);
    }
    else
    {
...
dump_counters(struct Client *source_p)
{
    ...
    DLINK_FOREACH(ptr, class_items.head)
    {
        ...
        DLINK_FOREACH(ptr2, classitem->list_ipv4.head)
        {
            ...
            ret = irc_getnameinfo((struct sockaddr *)&cidr->mask,
                                  cidr->mask.ss_len, ipaddr, HOSTIPLEN, NULL, 0,
                                  NI_NUMERICHOST);
            if(ret != 0)
                continue;

            sendto_one(source_p, ":%s %d %s n :cidr_table: %s: %s/%d %d",
                       me.name, RPL_STATSCCOUNT, source_p->name, conf->name,
                       ipaddr, CidrBitlenIPV4(classitem),
        ...
        DLINK_FOREACH(ptr2, classitem->list_ipv6.head)
        {
            ...
            sendto_one(source_p, ":%s %d %s n :cidr_table: %s: %s/%d %d",
                       me.name, RPL_STATSCCOUNT, source_p->name, conf->name,
                       ipaddr, CidrBitlenIPV6(classitem),
                       cidr->number_on_this_cidr);
        }
    }
}
    ...
    for(i = 0; i < ATABLE_SIZE; i++)
    {
        for(arec = atable[i]; arec; arec = arec->next)
        {
            if(arec->type == CONF_DLINE)
            {
                ...
                sendto_one(source_p, form_str(RPL_STATSDLINE), from, to, 'D',
...
stats_pending_glines(struct Client *source_p)
{
    ...
    {
        sendto_one(source_p,
                   ":%s NOTICE %s :This server does not support G-Lines", from,
                   to);
        return;
    }
    ...
    DLINK_FOREACH(pending_node, pending_glines.head)
    {
        ...
        sendto_one(source_p, ":%s NOTICE %s :1) %s!%s@%s on %s requested gline "
                             "at %s for %s@%s [%s]",
                   from, to, glp_ptr->oper_nick1, glp_ptr->oper_user1,
                   glp_ptr->oper_host1, glp_ptr->oper_server1, timebuffer,
        ...
        if(glp_ptr->oper_nick2[0] != '\0')
        {
            ...
            strftime(timebuffer, MAX_DATE_STRING, "%Y/%m/%d %H:%M:%S", tmptr);
            sendto_one(source_p, ":%s NOTICE %s :2) %s!%s@%s on %s requested "
                                 "gline at %s for %s@%s [%s]",
                       from, to, glp_ptr->oper_nick2, glp_ptr->oper_user2,
                       glp_ptr->oper_host2, glp_ptr->oper_server2, timebuffer,
...
#else
    sendto_one(source_p,
               ":%s NOTICE %s :This server does not support G-Line voting",
               from, to);
#endif /* GLINE VOTING */
}
    ...
    for(; i < ATABLE_SIZE; ++i)
    {
        for(arec = atable[i]; arec; arec = arec->next)
        {
            if(arec->type == CONF_GLINE)
            {
                ...
                sendto_one(source_p, form_str(RPL_STATSKLINE), from, to, "G",
...
stats_auth(struct Client *source_p)
{
    ...
    else if((ConfigFileEntry.stats_i_oper_only == 1) && !IsOper(source_p))
    {
        ...
        if(MyConnect(source_p))
            aconf = find_conf_by_address(
                source_p->host, &source_p->ip, CONF_CLIENT, source_p->aftype,
                source_p->username, source_p->localClient->passwd,
                source_p->certfp);
        else
            aconf = find_conf_by_address(source_p->host, NULL, CONF_CLIENT, 0,
        ...
        sendto_one(source_p, form_str(RPL_STATSILINE), from, to, 'I', "*",
                   show_iline_prefix(source_p, aconf, aconf->user), aconf->host,
                   aconf->port,
    ...
    else if((ConfigFileEntry.stats_k_oper_only == 1) && !IsOper(source_p))
    {
        ...
        if(MyConnect(source_p))
            aconf = find_conf_by_address(source_p->host, &source_p->ip,
                                         CONF_KILL, source_p->aftype,
                                         source_p->username, NULL, NULL);
        else
            aconf = find_conf_by_address(source_p->host, NULL, CONF_KILL, 0,
                                         source_p->username, NULL, NULL);
        ...
        sendto_one(source_p, form_str(RPL_STATSKLINE), from, to, "k",
                   aconf->host, aconf->user, aconf->reason, "");
    }
...
stats_klines(struct Client *source_p)
{
    ...
    else if((ConfigFileEntry.stats_k_oper_only == 1) && !IsOper(source_p))
    {
        ...
        sendto_one(source_p, form_str(RPL_STATSKLINE), from, to, "K",
                   aconf->host, aconf->user, aconf->reason, aconf->oper_reason);
    }
    /* Theyre opered, or allowed to see all klines */
    else
    {
...
stats_operedup(struct Client *source_p)
{
    ...
    DLINK_FOREACH(ptr, global_client_list.head)
    {
        ...
        if(MyClient(target_p))
        {
            if(MyClient(source_p) && IsOper(source_p))
                sendto_one(
                    source_p, ":%s %d %s p :[%c][%s] %s (%s@%s) Idle: %d", from,
                    RPL_STATSDEBUG, to,
                    IsAdmin(target_p)
                        ? (IsOperHiddenAdmin(target_p) ? 'O' : 'A')
                        : 'O',
    ...
    sendto_one(source_p, ":%s %d %s p :%lu OPER(s)", from, RPL_STATSDEBUG, to,
               oper_count);
}
...
stats_uptime(struct Client *source_p)
{
    ...
    sendto_one(source_p, form_str(RPL_STATSUPTIME), from, to, now / 86400,
...
stats_servers(struct Client *source_p)
{
    ...
    DLINK_FOREACH(ptr, serv_list.head)
    {
        ...
        sendto_one(source_p, ":%s %d %s v :%s (%s!%s@%s) Idle: %d", from,
                   RPL_STATSDEBUG, to, target_p->name,
                   (target_p->serv->by[0] ? target_p->serv->by : "Remote."),
                   "*", "*", (int)(CurrentTime - target_p->lasttime));
    }

    sendto_one(source_p, ":%s %d %s v :%d Server(s)", from, RPL_STATSDEBUG, to,
               j);
}
...
stats_ziplinks(struct Client *source_p)
{
    ...
    DLINK_FOREACH(ptr, serv_list.head)
    {
        ...
        if(IsCapable(target_p, CAP_ZIP))
        {
            ...
            sendto_one(
                source_p,
                ":%s %d %s Z :ZipLinks stats for %s send[%.2f%% "
                "compression (%lu bytes data/%lu bytes wire)] recv[%.2f%% "
                "compression (%lu bytes data/%lu bytes wire)]",
                from, RPL_STATSDEBUG, to, target_p->name, zipstats.out_ratio,
                zipstats.out, zipstats.out_wire, zipstats.in_ratio, zipstats.in,
                zipstats.in_wire);
            ++sent_data;
        }
    }

    sendto_one(source_p, ":%s %d %s Z :%u ziplink(s)", from, RPL_STATSDEBUG, to,
               sent_data);
}
...
stats_servlinks(struct Client *source_p)
{
    ...
    DLINK_FOREACH(ptr, serv_list.head)
    {
        ...
        sendto_one(
            source_p, form_str(RPL_STATSLINKINFO), from, to,
            ...
            (CurrentTime > target_p->since)
                ? (unsigned)(CurrentTime - target_p->since)
                : 0,
    ...
    sendto_one(source_p, ":%s %d %s ? :%u total server(s)", from,
               RPL_STATSDEBUG, to, dlink_list_length(&serv_list));
    sendto_one(source_p, ":%s %d %s ? :Sent total : %7.2f %s", from,
               RPL_STATSDEBUG, to, _GMKv(sendB), _GMKs(sendB));
    sendto_one(source_p, ":%s %d %s ? :Recv total : %7.2f %s", from,
               RPL_STATSDEBUG, to, _GMKv(recvB), _GMKs(recvB));

    uptime = (CurrentTime - me.since);

    sendto_one(
        source_p, ":%s %d %s ? :Server send: %7.2f %s (%4.1f K/s)", from,
        RPL_STATSDEBUG, to, _GMKv((me.localClient->send.bytes >> 10)),
        _GMKs((me.localClient->send.bytes >> 10)),
        (float)((float)((me.localClient->send.bytes) >> 10) / (float)uptime));
    sendto_one(
        source_p, ":%s %d %s ? :Server recv: %7.2f %s (%4.1f K/s)", from,
        RPL_STATSDEBUG, to, _GMKv((me.localClient->recv.bytes >> 10)),
 ...
 */
static void
ms_stats(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    if(hunt_server(client_p, source_p, ":%s STATS %s :%s", 2, parc, parv) !=
       HUNTED_ISME)
 ...
 */
static void
stats_L(struct Client *source_p, char *name, int doall, int wilds,
        char statchar)
{
             ...
             dlink_list *list, char statchar)
{
    ...
    DLINK_FOREACH(ptr, list->head)
    {
        ...
        if(IsInvisible(target_p) && (doall || wilds) &&
           !(MyConnect(source_p) && IsOper(source_p)) && !IsOper(target_p) &&
           (target_p != source_p))
            continue;
        ...
        {
            sendto_one(source_p, form_str(RPL_STATSLINKINFO), from, to,
                       (IsUpper(statchar)) ? get_client_name(target_p, SHOW_IP)
