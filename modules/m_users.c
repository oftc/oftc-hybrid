...
struct Message users_msgtab = {
    ...
    0,       0,
    0,       MFLG_SLOW,
    0,       {m_unregistered, m_users, mo_users, m_ignore, mo_users, m_ignore}};
 ...
 */
static void
m_users(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
 ...
 */
static void
mo_users(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    if(hunt_server(client_p, source_p, ":%s USERS :%s", 1, parc, parv) !=
       HUNTED_ISME)
