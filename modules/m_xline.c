 ...
 */
static void
mo_xline(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    char *reason                 = NULL;
    char *gecos                  = NULL;
    ...
    if(target_server != NULL)
    {
        ...
        if(tkline_time != 0)
            sendto_match_servs(source_p, target_server, CAP_ENCAP,
                               "ENCAP %s XLINE %s %s %d :%s", target_server,
                               target_server, gecos, (int)tkline_time, reason);
        else
            sendto_match_servs(source_p, target_server, CAP_CLUSTER,
                               "XLINE %s %s %d :%s", target_server, gecos,
                               (int)tkline_time, reason);
    ...
    else
    {
        if(tkline_time != 0)
            ...
            cluster_a_line(source_p, "XLINE", CAP_KLN, SHARED_XLINE, "%s 0 :%s",
                           gecos, reason);
    }
    ...
    if((conf = find_matching_name_conf(XLINE_TYPE, gecos, NULL, NULL, 0)) !=
       NULL)
    {
        match_item = map_to_conf(conf);

        sendto_one(source_p, ":%s NOTICE %s :[%s] already X-Lined by [%s] - %s",
                   me.name, source_p->name, gecos, conf->name,
                   match_item->reason);
        return;
    }
 ...
 */
static void
ms_xline(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
 ...
 */
static void
me_xline(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
...
relay_xline(struct Client *source_p, char *parv[], int encap)
{
    ...
    if(find_matching_name_conf(ULINE_TYPE, source_p->servptr->name,
                               ...
                               SHARED_XLINE))
    {
        if((conf = find_matching_name_conf(XLINE_TYPE, parv[2], NULL, NULL,
                                           0)) != NULL)
        {
            match_item = map_to_conf(conf);
            sendto_one(source_p,
 ...
 */
static void
mo_unxline(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
    char *gecos         = NULL;
    char *target_server = NULL;
    ...
    if(parse_aline("UNXLINE", source_p, parc, parv, 0, &gecos, NULL, NULL,
    ...
    if(target_server != NULL)
    {
        ...
        cluster_a_line(source_p, "UNXLINE", CAP_CLUSTER, SHARED_UNXLINE, "%s",
                       gecos);
 ...
 */
static void
ms_unxline(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
    if(parc != 3)
        return;
    ...
    sendto_match_servs(source_p, parv[1], CAP_CLUSTER, "UNXLINE %s %s", parv[1],
                       parv[2]);
...
valid_xline(struct Client *source_p, char *gecos, char *reason, int warn)
{
    ...
    if(strchr(gecos, '"'))
    {
        sendto_one(source_p, ":%s NOTICE %s :Invalid character '\"'", me.name,
                   source_p->name);
        return 0;
    }
    ...
    if(!valid_wild_card_simple(gecos))
    {
        if(warn)
            sendto_one(source_p, ":%s NOTICE %s :Please include at least %d "
                                 "non-wildcard characters with the xline",
...
remove_xline(struct Client *source_p, char *gecos)
{
    ...
    if(remove_txline_match(gecos))
    {
        ...
        ilog(L_NOTICE, "%s removed temporary X-Line for [%s]", source_p->name,
             gecos);
    ...
    if(remove_conf_line(XLINE_TYPE, source_p, gecos, NULL) > 0)
    {
        ...
        sendto_one(source_p, ":%s NOTICE %s :No X-Line for %s", me.name,
                   source_p->name, gecos);
}
