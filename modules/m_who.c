...
struct Message who2_msgtab = {
    ...
    0,     2,
    0,     MFLG_SLOW,
    0,     {m_unregistered, m_who, m_who, m_ignore, m_who, m_ignore}};
...
static struct flag_item who_user_modes[] = {
    {UMODE_ADMIN, 'a'},      {UMODE_BOTS, 'b'},
    {UMODE_CCONN, 'c'},      {UMODE_DEBUG, 'd'},
    {UMODE_FULL, 'f'},       {UMODE_CALLERID, 'g'},
    {UMODE_INVISIBLE, 'i'},  {UMODE_SKILL, 'k'},
    {UMODE_LOCOPS, 'l'},     {UMODE_NCHANGE, 'n'},
    {UMODE_OPER, 'o'},       {UMODE_REJ, 'r'},
    {UMODE_SERVNOTICE, 's'}, {UMODE_UNAUTH, 'u'},
    {UMODE_WALLOP, 'w'},     {UMODE_EXTERNAL, 'x'},
    {UMODE_SPY, 'y'},        {UMODE_OPERWALL, 'z'},
    {UMODE_GOD, 'S'},        {0, 0}};
...
build_searchopts(struct Client *source_p, int parc, char *parv[])
{
    ...
    if(parc < 1 || parv[0][0] == '?')
    {
        char **ptr = who_help;
        for(; *ptr; ptr++)
            sendto_one(source_p, form_str(RPL_WHOHELP), me.name, source_p->name,
                       *ptr);
    ...
    if(wsopts.search_chan &&
       !(wsopts.check_away || wsopts.gcos || wsopts.host ||
         wsopts.check_umode || wsopts.server || wsopts.user))
    {
        ...
        if(wsopts.show_chan &&
           !(wsopts.check_away || wsopts.gcos || wsopts.host ||
             wsopts.check_umode || wsopts.server || wsopts.user ||
             wsopts.nick || wsopts.ip || wsopts.channel))
    {
        if(parv[args] == NULL)
        {
            sendto_one(source_p, form_str(ERR_WHOSYNTAX), me.name,
...
m_who(struct Client *client_p, struct Client *source_p, int parc, char *parv[])
{
    ...
    else
    {
        DLINK_FOREACH(ptr, global_client_list.head)
        {
            ...
            status[1] =
                (IsOper(target_p)
                     ? '*'
                     : (IsInvisible(target_p) && IsOper(source_p) ? '%' : 0));
            status[2] = 0;
            sendto_one(
                source_p, form_str(RPL_WHOREPLY), me.name, source_p->name,
                wsopts.show_chan ? first_visible_channel(target_p, source_p)
                                 : "*",
                target_p->username,
                wsopts.show_ip
                    ? ((target_p->realhost[0] != '\0' && !IsOper(source_p)) ||
    ...
    sendto_one(
        source_p, form_str(RPL_ENDOFWHO), me.name, source_p->name,
        (wsopts.host != NULL
             ? wsopts.host
             : (wsopts.nick != NULL
                    ? wsopts.nick
