...
struct Message trace_msgtab = {
    ...
    0,       0,
    0,       MFLG_SLOW,
    0,       {m_unregistered, m_trace, ms_trace, m_ignore, mo_trace, m_ignore}};
 ...
 */
static void
m_trace(struct Client *client_p, struct Client *source_p, int parc,
        char *parv[])
{
    const char *tname;
 ...
 */
static void
mo_trace(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    dlink_node *ptr;
    const char *tname;
...
do_actual_trace(struct Client *source_p, int parc, char **parv)
{
    ...
    /* lets also do this for opers tracing nicks */
    {
        ...
        if(target_p && IsClient(target_p))
        {
            ...
            if(IsOper(target_p))
            {
                sendto_one(source_p, form_str(RPL_TRACEOPERATOR), from, to,
                           class_name, name,
            ...
            {
                sendto_one(source_p, form_str(RPL_TRACEUSER), from, to,
                           class_name, name,
                           IsIPSpoof(target_p) ? "255.255.255.255" : ipaddr,
                           CurrentTime - target_p->lasttime,
                           CurrentTime - target_p->localClient->last);
    ...
    DLINK_FOREACH(ptr, class_items.head)
    {
        ...
        if(CurrUserCount(cltmp) > 0)
            sendto_one(source_p, form_str(RPL_TRACECLASS), from, to, conf->name,
                       CurrUserCount(cltmp));
    }
...
*/
static void
ms_trace(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    if(hunt_server(client_p, source_p, ":%s TRACE %s :%s", 2, parc, parv))
        return;
                   ...
                   int link_u_p, int link_s_p)
{
    ...
    case STAT_CONNECTING:
        sendto_one(source_p, form_str(RPL_TRACECONNECTING), from, to,
                   class_name, IsOperAdmin(source_p) ? name : target_p->name);

        cnt++;
        break;
    case STAT_HANDSHAKE:
        sendto_one(source_p, form_str(RPL_TRACEHANDSHAKE), from, to, class_name,
    ...
    case STAT_UNKNOWN:
        /* added time -Taner */
        sendto_one(
            source_p, form_str(RPL_TRACEUNKNOWN), from, to, class_name, name,
            ip, target_p->firsttime ? CurrentTime - target_p->firsttime : -1);
        cnt++;
        break;
    case STAT_CLIENT:
           ...
           !dow || IsOper(target_p))
        {
            ...
            else if(IsOper(target_p))
            {
                if(ConfigFileEntry.hide_spoof_ips)
                    ...
                    sendto_one(source_p, form_str(RPL_TRACEOPERATOR), from, to,
                               class_name, name,
                               MyOper(source_p) ? ip : (IsIPSpoof(target_p)
                                                            ? "255.255.255.255"
                                                            : ip),
                               CurrentTime - target_p->lasttime,
                               CurrentTime - target_p->localClient->last);
            ...
            else
            {
                ...
                if(ConfigFileEntry.hide_spoof_ips)
                    sendto_one(source_p, format_str, from, to, class_name, name,
                               IsIPSpoof(target_p) ? "255.255.255.255" : ip,
                               CurrentTime - target_p->lasttime,
                               CurrentTime - target_p->localClient->last);
                else
    ...
    case STAT_SERVER:
        ...
        sendto_one(source_p, form_str(RPL_TRACESERVER), from, to, class_name,
                   link_s_p, link_u_p, name,
