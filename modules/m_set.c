...
struct Message set_msgtab = {
    ...
    0,     0,
    0,     MFLG_SLOW,
    0,     {m_unregistered, m_not_oper, m_error, m_ignore, mo_set, m_ignore}};
...
list_quote_commands(struct Client *source_p)
{
    ...
    for(i = 0; set_cmd_table[i].handler; i++)
    {
        ...
        if(j > 3)
        {
            sendto_one(source_p, ":%s NOTICE %s :%s %s %s %s", me.name,
                       source_p->name, names[0], names[1], names[2], names[3]);
            j        = 0;
            names[0] = names[1] = names[2] = names[3] = "";
        }
    }
...
quote_autoconnall(struct Client *source_p, int newval)
{
    if(newval >= 0)
    {
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             "%s has changed AUTOCONNALL to %i", source_p->name,
                             newval);
...
quote_idletime(struct Client *source_p, int newval)
{
    ...
    {
        sendto_one(source_p, ":%s NOTICE %s :IDLETIME is currently %i", me.name,
                   source_p->name, GlobalSetOptions.idletime / 60);
    }
}
...
quote_max(struct Client *source_p, int newval)
{
    if(newval > 0)
    {
        ...
        {
            sendto_one(source_p, ":%s NOTICE %s :You cannot set MAXCLIENTS to "
                                 "> %d, restoring to %d",
        ...
        if(newval < MAXCLIENTS_MIN)
        {
            sendto_one(source_p, ":%s NOTICE %s :You cannot set MAXCLIENTS to "
                                 "< %d, restoring to %d",
                       me.name, source_p->name, MAXCLIENTS_MIN,
...
quote_msglocale(struct Client *source_p, char *locale)
{
    if(locale != NULL)
    {
        ...
        sendto_one(source_p, ":%s NOTICE %s :Set MSGLOCALE to '%s'", me.name,
                   source_p->name, get_locale());
    }
    else
...
quote_spamnum(struct Client *source_p, int newval)
{
    if(newval >= 0)
    {
        ...
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             "%s has changed SPAMNUM to %i", source_p->name,
                             GlobalSetOptions.spam_num);
    }
    else
        sendto_one(source_p, ":%s NOTICE %s :SPAMNUM is currently %i", me.name,
...
quote_spamtime(struct Client *source_p, int newval)
{
    if(newval > 0)
    {
        ...
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             "%s has changed SPAMTIME to %i", source_p->name,
                             GlobalSetOptions.spam_time);
    }
    else
        sendto_one(source_p, ":%s NOTICE %s :SPAMTIME is currently %i", me.name,
                   source_p->name, GlobalSetOptions.spam_time);
}

/* this table is what splitmode may be set to */
static const char *splitmode_values[] = {"OFF", "ON", "AUTO", NULL};

/* this table is what splitmode may be */
static const char *splitmode_status[] = {"OFF", "AUTO (OFF)", "ON", "AUTO (ON)",
                                         NULL};
...
quote_splitmode(struct Client *source_p, char *charval)
{
    if(charval)
    {
        ...
        else if(newval == 1)
        {
            ...
            /* we might be deactivating an automatic splitmode, so pull the
             * event */
            eventDelete(check_splitmode, NULL);
        }
        /* AUTO */
...
quote_splitnum(struct Client *source_p, int newval)
{
    ...
    {
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             "%s has changed SPLITNUM to %i", source_p->name,
                             newval);
        ...
        sendto_one(source_p, ":%s NOTICE %s :SPLITNUM is currently %i", me.name,
                   source_p->name, split_servers);
}
...
quote_splitusers(struct Client *source_p, int newval)
{
    ...
    {
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             "%s has changed SPLITUSERS to %i", source_p->name,
                             newval);
        split_users = newval;
...
quote_jfloodtime(struct Client *source_p, int newval)
{
    if(newval >= 0)
    {
        sendto_realops_flags(UMODE_ALL, L_ALL,
                             "%s has changed JFLOODTIME to %i", source_p->name,
                             newval);
...
quote_jfloodcount(struct Client *source_p, int newval)
{
    ...
    {
        sendto_realops_flags(UMODE_ALL, L_ALL,
                             "%s has changed JFLOODCOUNT to %i", source_p->name,
                             newval);
...
quote_rejecttime(struct Client *source_p, int newval)
{
    if(newval >= 0)
    {
        ...
        sendto_one(source_p,
                   ":%s NOTICE %s :REJECTTIME is currently %i seconds", me.name,
                   source_p->name, GlobalSetOptions.rejecttime);
}
