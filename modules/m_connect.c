 ...
 */
static void
mo_connect(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
    int port;
    int tmpport;
    ...
    if(hunt_server(client_p, source_p, ":%s CONNECT %s %s :%s", 3, parc,
                   parv) != HUNTED_ISME)
        return;
     ...
     */
    if((conf = find_matching_name_conf(SERVER_TYPE, parv[1], NULL, NULL, 0)) !=
       NULL)
        aconf = (struct AccessItem *)map_to_conf(conf);
    else if((conf = find_matching_name_conf(SERVER_TYPE, NULL, NULL, parv[1],
                                            0)) != NULL)
        aconf = (struct AccessItem *)map_to_conf(conf);
     ...
     */
    ilog(L_TRACE, "CONNECT From %s : %s %s", source_p->name, parv[1],
         parv[2] ? parv[2] : "");
    ...
    if(serv_connect(aconf, source_p))
    {
        if(!ConfigServerHide.hide_server_ips && IsAdmin(source_p))
            sendto_one(source_p, ":%s NOTICE %s :*** Connecting to %s[%s].%d",
                       me.name, source_p->name, aconf->host, conf->name,
                       aconf->port);
        else
 ...
 */
static void
ms_connect(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
    int port;
    int tmpport;
     ...
     */
    if(serv_connect(aconf, source_p))
        sendto_one(source_p, ":%s NOTICE %s :*** Connecting to %s.%d", me.name,
                   source_p->name, conf->name, aconf->port);
    else
        sendto_one(source_p, ":%s NOTICE %s :*** Couldn't connect to %s.%d",
                   me.name, source_p->name, conf->name, aconf->port);
    /* client is either connecting with all the data it needs or has been
