 ...
 *  USA
 *
 *  $Id: fileio.c 33 2005-10-02 20:50:00Z knight $
 */
...
file_open(fde_t *F, const char *filename, int mode, int fmode)
{
    ...
    fd = (int)CreateFile(
        filename, dwDesiredAccess,
        FILE_SHARE_READ |
            ((dwDesiredAccess & GENERIC_WRITE) ? 0 : FILE_SHARE_WRITE),
        NULL, ((mode & O_CREAT) == 0)
                  ? OPEN_EXISTING
...
save_spare_fd(const char *spare_purpose)
{
    ...
    if(spare_fd < 0)
    {
        ilog(L_NOTICE, "Failed to reserve low fd for %s - open failed",
             spare_purpose);
        return -1;
    }
    else if(spare_fd > 255)
    {
        ilog(L_NOTICE, "Failed to reserve low fd for %s - too high",
             spare_purpose);
        close(spare_fd);
        return -1;
    }
