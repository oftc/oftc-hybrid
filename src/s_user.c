...
static void report_and_set_user_flags(struct Client *,
...
show_lusers(struct Client *source_p)
{
    ...
    if(dlink_list_length(&unknown_list) > 0)
        sendto_one(source_p, form_str(RPL_LUSERUNKNOWN), from, to,
                   dlink_list_length(&unknown_list));

    if(dlink_list_length(&global_channel_list) > 0)
        sendto_one(source_p, form_str(RPL_LUSERCHANNELS), from, to,
                   dlink_list_length(&global_channel_list));

    if(!ConfigServerHide.hide_servers || IsOper(source_p))
    {
        sendto_one(source_p, form_str(RPL_LUSERME), from, to, Count.local,
                   Count.myserver);
        sendto_one(source_p, form_str(RPL_LOCALUSERS), from, to, Count.local,
                   Count.max_loc, Count.local, Count.max_loc);
    }
    else
    {
        sendto_one(source_p, form_str(RPL_LUSERME), from, to, Count.total, 0);
        sendto_one(source_p, form_str(RPL_LOCALUSERS), from, to, Count.total,
                   Count.max_tot, Count.total, Count.max_tot);
    }

    sendto_one(source_p, form_str(RPL_GLOBALUSERS), from, to, Count.total,
               Count.max_tot, Count.total, Count.max_tot);
                    ...
                    const char *nick, const char *username)
{
    ...
    if(!IsGotId(source_p))
    {
        ...
        if(IsNeedIdentd(aconf))
        {
            ServerStats->is_ref++;
            sendto_one(source_p,
                       ":%s NOTICE %s :*** Notice -- You need to install "
    ...
    sendto_gnotice_flags(UMODE_CCONN, L_ALL, me.name, &me, NULL,
                         "Client connecting: %s (%s@%s) [%s] {%s} [%s]", nick,
                         source_p->username, source_p->host,
    ...
    sendto_realops_flags(UMODE_CCONN_FULL, L_ALL,
                         "CLICONN %s %s %s %s %s %s %s 0 %s", nick,
                         source_p->username, source_p->host,
                         ConfigFileEntry.hide_spoof_ips && IsIPSpoof(source_p)
                             ? "255.255.255.255"
                             : ipaddr,
                         ...
                         ConfigFileEntry.hide_spoof_ips && IsIPSpoof(source_p)
                             ? "<hidden>"
                             : source_p->client_host,
                         ConfigFileEntry.hide_spoof_ips && IsIPSpoof(source_p)
                             ? "<hidden>"
                             : source_p->client_server,
                         source_p->info);
    ...
    {
        free_dlink_node(m);
        dlinkAdd(source_p, &source_p->localClient->lclient_node,
                 &local_client_list);
    }
    else
        assert(0);
                     ...
                     const char *realname)
{
    ...
    {
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             "No server %s for user %s[%s@%s] from %s", server,
    ...
    if((target_p = source_p->servptr) && target_p->from != source_p->from)
    {
        ...
        kill_client(client_p, source_p,
                    "%s (NICK from wrong direction (%s != %s))", me.name,
                    source_p->servptr->name, target_p->from->name);
        SetKilled(source_p);
    ...
    aconf = find_conf_by_address(source_p->host, &source_p->ip, CONF_CLIENT,
                                 source_p->aftype, source_p->username, NULL,
                                 source_p->certfp);
    ...
    DLINK_FOREACH(server_node, serv_list.head)
    {
        ...
        if(IsCapable(server, CAP_TS6) && HasID(source_p))
            sendto_one(server, ":%s UID %s %d %lu %s %s %s %s %s :%s",
                       ...
                       source_p->hopcount + 1, (unsigned long)source_p->tsinfo,
            ...
            sendto_one(server, "NICK %s %d %lu %s %s %s %s :%s", source_p->name,
                       source_p->hopcount + 1, (unsigned long)source_p->tsinfo,
                       ubuf, source_p->username, source_p->host,
                       source_p->servptr->name, source_p->info);

        if(!EmptyString(source_p->certfp))
            sendto_one(server, "CERTFP %s %s", source_p->name,
                       source_p->certfp);
    }
}
 ...
 */
static void
report_and_set_user_flags(struct Client *source_p,
                          const struct AccessItem *aconf)
{
    /* If this user is being spoofed, tell them so */
    if(IsConfDoSpoofIp(aconf))
    {
        sendto_one(source_p, ":%s NOTICE %s :*** Spoofing your IP. congrats.",
                   me.name, source_p->name);
    }
...
do_local_user(const char *nick, struct Client *client_p,
              struct Client *source_p, const char *username, const char *host,
              const char *server, const char *realname)
{
    assert(source_p != NULL);
    assert(source_p->username != username);
 ...
 */
void
set_user_mode(struct Client *client_p, struct Client *source_p, int parc,
              char *parv[])
{
    ...
    {
        sendto_one(source_p, form_str(ERR_USERSDONTMATCH), me.name,
                   source_p->name);
        return;
    }
    ...
    if(parc < 3)
    {
        ...
        *m           = '\0';
        ...
        sendto_one(source_p, form_str(RPL_UMODEIS), me.name, source_p->name,
                   buf);
        return;
    }
    ...
    if(badflag)
        sendto_one(source_p, form_str(ERR_UMODEUNKNOWNFLAG), me.name,
                   source_p->name);
 ...
 */
void
send_umode(struct Client *client_p, struct Client *source_p, unsigned int old,
           unsigned int sendmask, char *umode_buf)
{
    int what = 0;
    unsigned int i;
    unsigned int flag;
    ...
    *m = '\0';
    ...
    if(*umode_buf && client_p)
        sendto_one(client_p, ":%s!%s@%s MODE %s :%s", source_p->name,
               ...
               unsigned int old)
{
    ...
    send_umode(NULL, source_p, old,
               IsOperHiddenAdmin(source_p) ? SEND_UMODES & ~UMODE_ADMIN
                                           : SEND_UMODES,
               buf);
...
user_welcome(struct Client *source_p)
{
    ...
    if(ConfigFileEntry.short_motd)
    {
        ...
        sendto_one(
            ...
            "read it",
            source_p->name);
        sendto_one(source_p, form_str(RPL_MOTDSTART), me.name, source_p->name,
                   me.name);
...
check_xline(struct Client *source_p)
{
    ...
    if((conf = find_matching_name_conf(XLINE_TYPE, source_p->info, NULL, NULL,
                                       0)) ||
       (conf = find_matching_name_conf(RXLINE_TYPE, source_p->info, NULL, NULL,
                                       0)))
    {
        ...
        sendto_gnotice_flags(
            UMODE_REJ, L_ALL, me.name, &me, NULL,
            "X-line Rejecting [%s] [%s], user %s [%s]", source_p->info, reason,
            get_client_name(source_p, HIDE_IP), source_p->sockhost);

        ServerStats->is_ref++;
        if(REJECT_HOLD_TIME > 0)
        {
            sendto_one(source_p, ":%s NOTICE %s :Bad user info", me.name,
                       source_p->name);
...
oper_up(struct Client *source_p, const char *name)
{
    ...
    sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                         "%s (%s@%s) is now an operator(%s)", source_p->name,
...
rebuild_isupport_message_line(void)
{
    ...
    DLINK_FOREACH(ptr, support_list.head)
    {
        ...
        *p = '\0';
        ...
        if(++tokens == (MAXPARA - 2) ||
           len >= (sizeof(isupportbuffer) - reserve))
        { /* arbritrary for now */
            if(*--p == ' ')
                *p = '\0';
...
check_godmode(void *unused)
{
    ...
    DLINK_FOREACH(ptr, oper_list.head)
    {
        ...
        if(IsGod(oper_p) &&
           (CurrentTime - oper_p->umodestime) > ConfigFileEntry.godmode_timeout)
        {
            ClearGod(oper_p);
