...
init_hash(void)
{
     ...
     */
    userhost_heap =
        BlockHeapCreate("userhost", sizeof(struct UserHost), CLIENT_HEAP_SIZE);
    namehost_heap =
        BlockHeapCreate("namehost", sizeof(struct NameHost), CLIENT_HEAP_SIZE);
 ...
 */
void
count_user_host(const char *user, const char *host, int *global_p, int *local_p,
                int *icount_p)
{
    dlink_node *ptr;
...
dump_userhosttable(struct Client *source_p)
{
    ...
    for(i = 0; i < HASHSIZE; ++i)
    {
        for(userhost = userhostTable[i]; userhost != NULL;
            userhost = userhost->next)
        {
            DLINK_FOREACH(ptr, userhost->list.head)
            {
                ...
                sendto_one(source_p,
                           ...
                           me.name, RPL_STATSCCOUNT, source_p->name,
                           nameh->name, userhost->host, nameh->icount,
                           nameh->gcount, nameh->lcount);
            }
        }
    }
}
                 ...
                 struct ListTask *list_task, int remote_request)
{
    if((remote_request && chptr->chname[0] == '&') ||
       (SecretChannel(chptr) && !IsMember(source_p, chptr)))
        return;
    if((unsigned int)dlink_list_length(&chptr->members) <
           list_task->users_min ||
       (unsigned int)dlink_list_length(&chptr->members) >
           list_task->users_max ||
       (chptr->channelts != 0 &&
