            ...
            struct Client *server_p)
{
    ...
    sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                         "SlinkError for %s: %s", server_p->name, data);
               ...
               struct Client *server_p)
{
     ...
     * -jmallett, 04/27/2002
     */
    memcpy(&zipstats, &server_p->localClient->zipstats,
...
#define ASNO(a, b) a = (a + b >= a ? a + b : b)
    ...
    if(zipstats.in > 0)
        zipstats.in_ratio =
            (((double)(zipstats.in - zipstats.in_wire) / (double)zipstats.in) *
             100.00);
    else
        zipstats.in_ratio = 0;
    ...
    memcpy(&server_p->localClient->zipstats, &zipstats,
           sizeof(struct ZipStats));
}
...
write_links_file(void *notused)
{
    ...
    DLINK_FOREACH(ptr, global_serv_list.head)
    {
        ...
        /* Attempt to format the file in such a way it follows the usual links
         * output
         ...
         * This should be changed to an snprintf at some point, but I'm wanting
         * to
         * know if this is a cause of a bug - cryogen
         */
        assert(strlen(target_p->name) + strlen(me.name) + 6 + strlen(p) <=
               MESSAGELINELEN);
        ircsprintf(newMessageLine->line, "%s %s :1 %s", target_p->name, me.name,
                   p);
            ...
            const char *command, int server, int parc, char *parv[])
{
    ...
    if(target_p != NULL)
    {
        ...
        {
            sendto_one(source_p, form_str(ERR_NOSUCHSERVER), me.name, parv[0],
                       parv[server]);
            return HUNTED_NOSUCH;
        }
        ...
        sendto_one(target_p, command, parv[0], parv[1], parv[2], parv[3],
            ...
            if((cryptlink && IsConfCryptLink(aconf)) || IsConfSSLLink(aconf))
            {
                if(aconf->rsa_public_key)
                {
                    ...
                    if(IsConfSSLLink(aconf))
                    {
                        ...
                        if((cert = SSL_get_peer_certificate(
                                client_p->localClient->fd.ssl)) == NULL)
                            return -2;

                        ret = SSL_get_verify_result(
                           ...
                           ret == X509_V_ERR_CERT_UNTRUSTED)
                        {
                            ...
                            if(get_rsa_fingerprint(aconf->rsa_public_key,
                                                   our_fp) != 0)
                                return -2;
                            if(get_rsa_fingerprint(cert_rsa, their_fp) != 0)
                                return -2;

                            ilog(L_DEBUG, "Comparing '%s' and '%s'", our_fp,
                                 their_fp);
...
sendnick_TS(struct Client *client_p, struct Client *target_p)
{
    ...
    send_umode(NULL, target_p, 0,
               IsOperHiddenAdmin(target_p) ? SEND_UMODES & ~UMODE_ADMIN
                                           : SEND_UMODES,
               ubuf);
    ...
    if(HasID(target_p) && IsCapable(client_p, CAP_TS6))
        sendto_one(client_p, ":%s UID %s %d %lu %s %s %s %s %s :%s",
                   ...
                   target_p->hopcount + 1, (unsigned long)target_p->tsinfo,
        ...
        sendto_one(client_p, "NICK %s %d %lu %s %s %s %s :%s", target_p->name,
                   target_p->hopcount + 1, (unsigned long)target_p->tsinfo,
                   ubuf, target_p->username, target_p->host,
...
#ifdef HAVE_LIBCRYPTO
    if(IsCapable(target_p, CAP_ENC) && target_p->localClient->in_cipher &&
...
server_estab(struct Client *client_p)
{
    ...
    inpath =
        get_client_name(client_p, MASK_IP); /* "refresh" inpath with host */
    host = client_p->name;

    if((conf = find_conf_name(&client_p->localClient->confs, host,
                              SERVER_TYPE)) == NULL)
    {
        /* This shouldn't happen, better tell the ops... -A1kmm */
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             "Warning: Lost connect{} block "
                             "for server %s(this shouldn't happen)!",
                             host);
    ...
    if(IsUnknown(client_p) && !IsConfCryptLink(aconf))
    {
        ...
        else if(!EmptyString(aconf->spasswd))
        {
            ...
            if(me.id[0] != '\0') /* Send TS 6 form only if id */
                sendto_one(client_p, "PASS %s TS %d %s", aconf->spasswd,
                           TS_CURRENT, me.id);
            else
         ...
         */
        sendto_one(client_p, "SERVER %s 1 :%s%s", my_name_for_link(conf),
    ...
    if(IsCapable(client_p, CAP_ENC) || IsCapable(client_p, CAP_ZIP))
    {
        ...
        {
            sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                                 "Warning: fork failed for server %s -- check "
                                 "servlink_path (%s)",
    ...
    sendto_one(client_p, "SVINFO %d %d 0 :%lu", (me.id[0] ? TS_CURRENT : 5),
               TS_MIN, (unsigned long)CurrentTime);

    /* assumption here is if they passed the correct TS version, they also
     * passed an SID */
    ...
    */
    ...
    sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                         "Link with %s established: (%s) link", inpath,
                         show_capabilities(client_p));
    ilog(L_NOTICE, "Link with %s established: (%s) link", inpath_ip,
         show_capabilities(client_p));
    ...
    DLINK_FOREACH(ptr, serv_list.head)
    {
        ...
        if(IsCapable(target_p, CAP_TS6) && HasID(client_p))
            sendto_one(target_p, ":%s SID %s 2 %s :%s%s", me.id, client_p->name,
                       client_p->id, IsHidden(client_p) ? "(H) " : "",
                       client_p->info);
        else
            sendto_one(target_p, ":%s SERVER %s 2 :%s%s", me.name,
                       client_p->name, IsHidden(client_p) ? "(H) " : "",
                       client_p->info);
    }
...
start_io(struct Client *server)
{
    ...
    if(IsCapable(server, CAP_ENC))
        alloclen +=
            16 + lserver->in_cipher->keylen + lserver->out_cipher->keylen;
#endif
...
burst_all(struct Client *client_p)
{
    ...
    /* We send the time we started the burst, and let the remote host determine
    *an EOB time,
    ** as otherwise we end up sending a EOB of 0   Sending here means it gets
    *sent last -- fl
    */
...
send_tb(struct Client *client_p, struct Channel *chptr)
{
    ...
    if(chptr->topic_time != 0)
    {
        if(IsCapable(client_p, CAP_TBURST))
            sendto_one(client_p, ":%s TBURST %lu %s %lu %s :%s", me.name,
        ...
        else if(IsCapable(client_p, CAP_TB))
        {
            ...
            {
                sendto_one(client_p, ":%s TB %s %lu %s :%s", me.name,
                           chptr->chname, (unsigned long)chptr->topic_time,
                           chptr->topic_info, chptr->topic ? chptr->topic : "");
            }
            else
            {
                sendto_one(client_p, ":%s TB %s %lu :%s", me.name,
                           chptr->chname, (unsigned long)chptr->topic_time,
                           chptr->topic ? chptr->topic : "");
            }
        }
...
burst_channel(struct Client *client_p, struct Channel *chptr)
{
    ...
    {
        sendto_one(client_p, ":%s TOPIC %s %s %lu :%s", me.name, chptr->chname,
                   chptr->topic_info, (unsigned long)chptr->topic_time,
                   chptr->topic);
    }
}
...
set_autoconn(struct Client *source_p, const char *name, int newval)
{
    ...
    if(name != NULL)
    {
        ...
        {
            sendto_one(source_p, ":%s NOTICE %s :Can't find %s", me.name,
                       source_p->name, name);
        }
    }
    else
...
serv_connect(struct AccessItem *aconf, struct Client *by)
{
    ...
    /* log */
    irc_getnameinfo((struct sockaddr *)&aconf->ipnum, aconf->ipnum.ss_len, buf,
    ...
    {
        sendto_gnotice_flags(UMODE_ALL, L_OPER, me.name, &me, NULL,
                             "Error connecting to %s: Error during DNS lookup",
                             conf->name);
        return (0);
    }
    ...
    {
        sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                             "Server %s already present from %s", conf->name,
    ...
    if(!attach_connect_block(client_p, conf->name, aconf->host))
    {
        sendto_gnotice_flags(
            ...
            "Host %s is not enabled for connecting:no C/N-line", conf->name);
        if(by && IsClient(by) && !MyClient(by))
            sendto_one(by, ":%s NOTICE %s :Connect to host %s failed.", me.name,
                       by->name, client_p->name);
        SetDead(client_p);
    ...
    case AF_INET:
        ...
        if(v4->sin_addr.s_addr != 0)
        {
            ...
            comm_connect_tcp(&client_p->localClient->fd, aconf->host,
                             aconf->port, (struct sockaddr *)&ipn, ipn.ss_len,
...
serv_connect_callback(fde_t *fd, int status, void *data)
{
    ...
    memcpy(&client_p->ip, &fd->connect.hostaddr, sizeof(struct irc_ssaddr));
    /* Check the status */
    if(status != COMM_OK)
    {
    ...
    /* Get the C/N lines */
    conf = find_conf_name(&client_p->localClient->confs, client_p->name,
                          SERVER_TYPE);
    if(conf == NULL)
    {
        sendto_gnotice_flags(UMODE_ALL, L_ADMIN, me.name, &me, NULL,
    ...
    if(!EmptyString(aconf->spasswd) && (me.id[0] != '\0'))
        ...
        sendto_one(client_p, "PASS %s TS %d %s", aconf->spasswd, TS_CURRENT,
                   me.id);
    else
        sendto_one(client_p, "PASS %s TS 5", aconf->spasswd);
...
ssllink_init(struct Client *client_p, struct ConfItem *conf, fde_t *fd)
{
    ...
    cert   = create_certificate(pubkey, ServerInfo.rsa_private_key, me.name,
                              me.name, 2 * 60 * 60);
    RSA_free(pubkey);
...
cryptlink_init(struct Client *client_p, struct ConfItem *conf, fde_t *fd)
{
    ...
    enc_len   = RSA_public_encrypt(CIPHERKEYLEN, (unsigned char *)randkey,
    ...
    sendto_one(client_p, "CRYPTLINK SERV %s %s :%s%s", my_name_for_link(conf),
               key_to_send, ConfigServerHide.hidden ? "(H) " : "", me.info);
...
void
cryptlink_error(struct Client *client_p, const char *type, const char *reason,
