...
static int write_csv_line(FBFILE *, const char *, ...) __attribute__((format (printf, 2, 3)));
		...
		const char *current_date, time_t cur_time, time_t duration)
{
  ...
  case CRESV_TYPE:
    ...
    if(duration == 0)
      ...
      write_csv_line(out, "%s%s\"%ld\"", cresv_p->name, cresv_p->reason, cresv_p->hold);
    break;
  ...
  case NRESV_TYPE:
    ...
    if(duration == 0)
      ...
      write_csv_line(out, "%s%s\"%ld\"", conf->name, nresv_p->reason, nresv_p->hold);
    break;
