 ...
 *
 *  Modified 2001/11/29 for mmap() support by Aaron Sethman
 * <androsyn@ratbox.org>
 *
 ...
 * allocate may, 200MB of memory, while our normal memory consumption would be
...
block_heap_report_stats(struct Client *client_p)
{
    ...
    for(bh = heap_list; bh != NULL; bh = bh->next)
        sendto_one(
            ...
            block_heap_get_used_elm(bh), block_heap_get_used_mem(bh),
            block_heap_get_free_elm(bh), block_heap_get_free_mem(bh),
