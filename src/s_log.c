...
#ifdef USE_SYSLOG
static const int sysLogLevel[] = {LOG_CRIT, LOG_ERR,  LOG_WARNING, LOG_NOTICE,
        ...
        if(IsClient(source_p))
        {
            if(user_log_fb == NULL)
            {
                if((ConfigLoggingEntry.userlog[0] != '\0') &&
                   (user_log_fb = fbopen(ConfigLoggingEntry.userlog, "r")) !=
                       NULL)
                {
                    fbclose(user_log_fb);
            ...
            if(user_log_fb != NULL)
            {
                size_t nbytes = ircsprintf(
                    linebuf,
                    "%s (%3u:%02u:%02u): %s!%s@%s %" PRIu64 "/%" PRIu64 "\n",
                    myctime(source_p->firsttime), (unsigned int)(on_for / 3600),
                    (unsigned int)((on_for % 3600) / 60),
                    (unsigned int)(on_for % 60), source_p->name,
                ...
                const char *pattern, ...)
{
    ...
    if(source_p != NULL)
    {
        n_preamble = ircsprintf(
            linebuf, "%s %s by (%s!%s@%s) :", myctime(CurrentTime), log_message,
            source_p->name, source_p->username, source_p->host);
    }
    else
    {
        n_preamble =
            ircsprintf(linebuf, "%s %s :", myctime(CurrentTime), log_message);
    }
