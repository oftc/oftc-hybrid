...
struct logging_entry ConfigLoggingEntry = {1,   {0}, {0}, {0}, {0},
                                           {0}, {0}, {0}, {0}};
...
struct lgetopt myopts[] = {
    {"dlinefile", &ConfigFileEntry.dlinefile, STRING,
     "File to use for dline.conf"},
    {"configfile", &ConfigFileEntry.configfile, STRING,
     "File to use for ircd.conf"},
    {"klinefile", &ConfigFileEntry.klinefile, STRING,
     "File to use for kline.conf"},
    {"xlinefile", &ConfigFileEntry.xlinefile, STRING,
     "File to use for xline.conf"},
    {"logfile", &logFileName, STRING, "File to use for ircd.log"},
    {"pidfile", &pidFileName, STRING, "File to use for process ID"},
    {"foreground", &server_state.foreground, YESNO,
    ...
    while(1 == 1)
    {
        ...
        if(listing_client_list.head)
        {
            ...
            DLINK_FOREACH_SAFE(ptr, ptr_next, listing_client_list.head)
            {
                ...
                safe_list_channels(client_p, client_p->localClient->list_task,
                                   0, 0);
            }
        }
...
write_pidfile(const char *filename)
{
    ...
    if((fb = fbopen(filename, "w")))
    {
        ...
        if((fbputs(buff, fb, nbytes) == -1))
            ilog(L_ERROR, "Error writing %u to pid file %s (%s)", pid, filename,
                 strerror(errno));
...
init_ssl(void)
{
    ...
    SSL_CTX_set_verify(ServerInfo.ctx, SSL_VERIFY_PEER,
                       always_accept_verify_cb);
    SSL_CTX_set_session_cache_mode(ServerInfo.ctx, SSL_SESS_CACHE_OFF);
    SSL_CTX_set_cipher_list(ServerInfo.ctx,
