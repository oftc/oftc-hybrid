...
show_ports(struct Client *source_p)
{
    ...
    DLINK_FOREACH(ptr, ListenerPollList.head)
    {
        ...
        *p       = '\0';
        sendto_one(
            source_p, form_str(RPL_STATSPLINE), me.name, source_p->name, 'P',
            listener->port, IsAdmin(source_p) ? listener->name : me.name,
...
accept_connection(fde_t *pfd, void *data)
{
    ...
    comm_setselect(&listener->fd, COMM_SELECT_READ, accept_connection, listener,
                   0);
}
