...
static const char *core_module_table[] = {
    "m_die",  "m_join", "m_kick",   "m_kill",  "m_message", "m_mode", "m_nick",
    "m_part", "m_quit", "m_server", "m_sjoin", "m_squit",   NULL};

static const char *spy_module_table[] = {
    "spy_admin_notice", "spy_info_notice",  "spy_links_notice",
    "spy_motd_notice",  "spy_stats_notice", "spy_stats_p_notice",
    "spy_trace_notice", "spy_whois_notice", NULL};
...
load_core_modules(int warn)
{
    ...
    for(i = 0; spy_module_table[i]; ++i)
    {
        ...
        if(load_a_module(module_name, warn, 1) == -1)
        {
            ilog(L_CRIT, "Error loading spy module %s%s", spy_module_table[i],
                 SHARED_SUFFIX);
        }
    }
}
             ...
             char *parv[])
{
    ...
    {
        sendto_one(source_p, ":%s NOTICE %s :Module %s is not loaded", me.name,
                   source_p->name, m_bn);
        return;
    }
        ...
        if(parc > 1)
        {
            if(match(parv[1], modp->name))
            {
                sendto_one(source_p, form_str(RPL_MODLIST), me.name, parv[0],
                           modp->name, modp->address, modp->version,
                           modp->core ? "(core)" : "");
            }
        }
        else
              ...
              char *parv[])
{
    ...
    sendto_one(source_p, ":%s NOTICE %s :Reloading all modules", me.name,
               source_p->name);
