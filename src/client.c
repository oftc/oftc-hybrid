...
init_client(void)
{
     ...
     */
    client_heap =
        BlockHeapCreate("client", sizeof(struct Client), CLIENT_HEAP_SIZE);
    lclient_heap = BlockHeapCreate("local client", sizeof(struct LocalUser),
                                   LCLIENT_HEAP_SIZE);
        ...
        */
        ...
        if(ping < CurrentTime - client_p->lasttime)
        {
            ...
            else
            {
                ...
                {
                    /*
                     * If the client/server hasn't talked to us in 2*ping
                     * seconds
                ...
                /* We want to hear about missing ping replies every 5 (or
                 * whatever
                        ...
                        CurrentTime - client_p->lasttime >= ping + pingwarn)
                {
                    ...
                    strftime(timestamp, sizeof(timestamp),
                             "%Y-%m-%d %H:%M:%S Z", gmtime(&CurrentTime));
                    /*
                     * If the server hasn't replied in pingwarn seconds after
                     * sending
                     * the PING, notify the opers so that they are aware of the
                     * problem.
                     */
                    ...
                    sendto_gnotice_flags(
                        ...
                        get_client_name(client_p, SHOW_IP),
                        (CurrentTime - client_p->lasttime - ping), timestamp);
...
check_conf_klines(void)
{
    ...
    DLINK_FOREACH_SAFE(ptr, next_ptr, local_client_list.head)
    {
        ...
        if((aconf = find_dline_conf(&client_p->ip, client_p->aftype)) != NULL)
        {
            if(aconf->status & CONF_EXEMPTDLINE)
                continue;

            sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                                 "DLINE %s@%s (%s) active for %s", aconf->user,
        ...
        if(ConfigFileEntry.glines && (aconf = find_gline(client_p)))
        {
            ...
            sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                                 "GLINE %s@%s (%s) active for %s", aconf->user,
                                 aconf->host, aconf->reason,
                                 get_client_name(client_p, SHOW_IP));
        ...
        if((aconf = find_kill(client_p)) != NULL)
        {
            ...
            sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                                 "KLINE %s@%s (%s) active for %s", aconf->user,
                                 aconf->host, aconf->reason,
                                 get_client_name(client_p, SHOW_IP));
        ...
        if((conf = find_matching_name_conf(XLINE_TYPE, client_p->info, NULL,
                                           NULL, 0)) != NULL ||
           (conf = find_matching_name_conf(RXLINE_TYPE, client_p->info, NULL,
                                           NULL, 0)) != NULL)
        {
            aconf = map_to_conf(conf);
            sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                                 "XLINE %s@%s (%s) active for %s", aconf->user,
                                 aconf->host, aconf->reason,
                                 get_client_name(client_p, SHOW_IP));
    ...
    DLINK_FOREACH_SAFE(ptr, next_ptr, unknown_list.head)
    {
        ...
        if((aconf = find_dline_conf(&client_p->ip, client_p->aftype)))
        {
            if(aconf->status & CONF_EXEMPTDLINE)
                continue;
...
ban_them(struct Client *client_p, struct ConfItem *conf)
{
    ...
    sendto_realops_flags(UMODE_ALL, L_ALL, "%s active for %s", type_string,
                         get_client_name(client_p, HIDE_IP));

    if(IsClient(client_p))
        sendto_one(client_p, form_str(ERR_YOUREBANNEDCREEP), me.name,
                   client_p->name, user_reason);
 ...
 */
struct Client *
find_chasing(struct Client *client_p, struct Client *source_p, const char *user,
             int *chasing)
{
    ...
    if((who = get_history(
            user, (time_t)ConfigFileEntry.kill_chase_time_limit)) == NULL)
    {
        ...
        */
        ...
        if(!MyConnect(source_p))
        {
            aconf = find_conf_by_address(
                ...
                source_p->username, NULL, source_p->certfp);
                   ...
                   const char *splitstr, const char *myname)
{
    ...
    recurse_send_quits(original_source_p, ptr->data, from, to, comment,
                       splitstr, myname);
                  ...
                  const char *comment, const char *splitstr)
{
    ...
    DLINK_FOREACH(ptr, serv_list.head)
    {
        ...
        recurse_send_quits(source_p, source_p, from, to, comment, splitstr,
                           myname);
    }
...
exit_client(struct Client *source_p, struct Client *from, const char *comment)
{
    ...
    if(MyConnect(source_p))
    {
        ...
        else if(IsClient(source_p))
        {
            ...
            sendto_gnotice_flags(
                UMODE_CCONN, L_ALL, me.name, &me, NULL,
            ...
            sendto_realops_flags(
                UMODE_CCONN_FULL, L_ALL, "CLIEXIT: %s %s %s %s 0 %s",
        ...
        if(!IsDead(source_p))
        {
            ...
            sendto_one(source_p, "ERROR :Closing Link: %s (%s)", source_p->host,
                       comment);
        }
    ...
    if(IsServer(source_p))
    {
        ...
        if(source_p->servptr == &me)
        {
            ...
            ilog(L_NOTICE, "%s was connected for %d seconds.  %" PRIu64
                           "/%" PRIu64 " sendK/recvK.",
                 source_p->name, (int)(CurrentTime - source_p->firsttime),
                 source_p->localClient->send.bytes >> 10,
                 source_p->localClient->recv.bytes >> 10);
...
dead_link_on_read(struct Client *client_p, int error)
{
    ...
    if(IsServer(client_p) || IsHandshake(client_p))
    {
        ...
        sendto_gnotice_flags(
            UMODE_ALL, L_ALL, me.name, &me, NULL,
            "%s had been connected for %d day%s, %2d:%02d:%02d", client_p->name,
            connected / 86400, (connected / 86400 == 1) ? "" : "s",
                  ...
                  const char *nick)
{
    ...
    */
       ...
       (IsOper(source_p) && ConfigFileEntry.no_oper_flood))
    {
         ...
         */
        sendto_gnotice_flags(UMODE_NCHANGE, L_ALL, me.name, &me, NULL,
    ...
    {
        sendto_one(source_p, form_str(ERR_NICKTOOFAST), me.name, source_p->name,
