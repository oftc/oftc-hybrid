...
static const char *HeaderMessages[] = {
    ...
    ":%s NOTICE AUTH :*** No Ident response", ":%s NOTICE AUTH :*** Your "
                                              "forward and reverse DNS do not "
...
init_auth(void)
{
    ...
    eventAddIsh("timeout_auth_queries_event", timeout_auth_queries_event, NULL,
                1);
}
...
release_auth_client(struct Client *client)
{
    ...
    if((client->node.prev != NULL) || (client->node.next != NULL))
    {
        sendto_realops_flags(UMODE_ALL, L_OPER, "already linked %s at %s:%d",
                             client->name, __FILE__, __LINE__);
        ilog(L_ERROR, "already linked %s at %s:%d", client->name, __FILE__,
             __LINE__);
        assert(0 == 5);
    }
    else
...
start_auth_query(struct AuthRequest *auth)
{
    ...
    if(comm_open(&auth->client->localClient->auth_fd,
                 auth->client->ip.ss.ss_family, SOCK_STREAM, 0, "ident") == -1)
    {
    ...
    comm_connect_tcp(
        &auth->client->localClient->auth_fd, auth->client->sockhost, 113,
        (struct sockaddr *)&localaddr, localaddr.ss_len, auth_connect_callback,
        auth, auth->client->ip.ss.ss_family, GlobalSetOptions.ident_timeout);
