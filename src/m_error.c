              ...
              int parc, char *parv[])
{
  ...
  if (client_p == source_p)
    sendto_gnotice_flags(FLAGS_ALL, L_OPER, me.name, &me, NULL,"ERROR :from %s -- %s",
			 get_client_name(client_p, MASK_IP), para);
  else
    sendto_gnotice_flags(FLAGS_ALL, L_OPER, me.name, &me, NULL,"ERROR :from %s via %s -- %s", source_p->name,
			 get_client_name(client_p, MASK_IP), para);
}
