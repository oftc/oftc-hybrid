...
static const char *comm_err_str[] = {
    "Comm OK",         "Error during bind()",    "Error during DNS lookup",
    "connect timeout", "Error during connect()", "Comm Error"};
...
add_connection(struct Listener *listener, struct irc_ssaddr *irn, int fd)
{
    ...
    fd_open(&new_client->localClient->fd, fd, 1,
            (listener->flags & LISTENER_SSL) ? "Incoming SSL connection"
                                             : "Incoming connection");
    ...
    irc_getnameinfo((struct sockaddr *)&new_client->ip, new_client->ip.ss_len,
                    new_client->sockhost, HOSTIPLEN, NULL, 0, NI_NUMERICHOST);
    new_client->aftype = new_client->ip.ss.ss_family;
#ifdef IPV6
    if(new_client->sockhost[0] == ':')
        strlcat(new_client->host, "0", HOSTLEN + 1);

    if(new_client->aftype == AF_INET6 && ConfigFileEntry.dot_in_ip6_addr == 1)
    {
...
comm_connect_tryconnect(fde_t *fd, void *notused)
{
    ...
    if(retval < 0)
    {
        ...
        if(errno == EISCONN)
            ...
            comm_setselect(fd, COMM_SELECT_WRITE, comm_connect_tryconnect, NULL,
                           0);
        else
