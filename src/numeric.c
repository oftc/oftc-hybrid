...
change_reply(const char *locale, int linecnt, int reply, char *new_reply)
{
    ...
    for(; *new; new ++)
    {
        if(*new == '%')
        {
            ...
            if(*new != '%')
            {
                /* We've just found a format symbol. Check if it is the next
                 * format
...
set_locale(const char *locale)
{
    ...
    {
        strlcpy(used_locale, "standard",
                sizeof(used_locale)); /* XXX paranoid */
        return;
    }
    ...
    while(fbgets(buffer, sizeof(buffer), f))
    {
        ...
        if(i != -1)
        {
            ilog(L_ERROR, "Unknown numeric %s (%s.lang, %d)", ident, locale,
                 linecnt);
            res = 0;
        }
    }
