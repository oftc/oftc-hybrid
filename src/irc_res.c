 ...
 *
 * $Id: irc_res.c 468 2006-02-17 02:42:23Z db $
 *
...
static void do_query_name(struct DNSQuery *query, const char *name,
                          struct reslist *request, int);
static void do_query_number(struct DNSQuery *query, const struct irc_ssaddr *,
 ...
 */
static void
do_query_name(struct DNSQuery *query, const char *name, struct reslist *request,
              int type)
{
                ...
                struct reslist *request)
{
    ...
    if(addr->ss.ss_family == AF_INET)
    {
        ...
        ircsprintf(ipbuf, "%u.%u.%u.%u.in-addr.arpa.", (unsigned int)(cp[3]),
                   (unsigned int)(cp[2]), (unsigned int)(cp[1]),
                   (unsigned int)(cp[0]));
    }
#ifdef IPV6
    else if(addr->ss.ss_family == AF_INET6)
    {
        ...
        (void)sprintf(
            ipbuf, "%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x.%x."
