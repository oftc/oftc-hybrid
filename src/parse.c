...
static void handle_command(struct Message *, struct Client *, struct Client *,
                           unsigned int, char **);
static void recurse_report_messages(struct Client *source_p,
                                    struct MessageTree *mtree);
static void add_msg_element(struct MessageTree *mtree_p, struct Message *msg_p,
                            const char *cmd);
...
parse(struct Client *client_p, char *pbuffer, char *bufend)
{
    ...
    if(*ch == ':')
    {
        ...
        {
            /*
             * XXX it could be useful to know which of these occurs most
             * frequently.
             * the ID check should always come first, though, since it is so
             * easy.
             */
            if((from = find_person(client_p, sender)) == NULL)
            {
                ...
                if(from == NULL && IsCapable(client_p, CAP_TS6) &&
                   ...
                   strlen(sender) == 3)
                {
                    /* Dirty hack to allow messages from masked SIDs (i.e. the
                     * ones
                     * hidden by fakename="..."). It shouldn't break anything,
                     * since
               ...
               struct Client *from, unsigned int i, char *hpara[MAXPARA])
{
    ...
    if(i < mptr->parameters)
    {
        ...
        else
        {
            ...
            ilog(L_CRIT,
                 "Insufficient parameters (%d) for command '%s' from %s.", i,
                 mptr->cmd, client_p->name);
                ...
                const char *cmd)
{
    ...
    else
    {
        ...
        {
            ntree_p =
                (struct MessageTree *)MyMalloc(sizeof(struct MessageTree));
...
recurse_report_messages(struct Client *source_p, struct MessageTree *mtree)
{
    ...
    if(mtree->msg != NULL)
    {
        sendto_one(source_p, form_str(RPL_STATSCOMMANDS), me.name,
                   source_p->name, mtree->msg->cmd, mtree->msg->count,
                   mtree->msg->bytes, mtree->msg->rcount);
    }
...
cancel_clients(struct Client *client_p, struct Client *source_p, char *cmd)
{
    ...
    if(IsServer(source_p) || IsMe(source_p))
    {
        sendto_gnotice_flags(UMODE_DEBUG, L_ALL, me.name, &me, NULL,
                             "Message for %s[%s] from %s", source_p->name,
                             source_p->from->name,
...
remove_unknown(struct Client *client_p, char *lsender, char *lbuffer)
{
       ...
       strchr(lsender, '.') != NULL)
    {
        ...
        sendto_one(client_p, ":%s KILL %s :%s (Unknown Client)", me.name,
                   lsender, me.name);
}
           ...
           int parc, char *parv[])
{
       ...
       ((target_p = find_server(parv[1])) != NULL))
    {
        if(IsMe(target_p))
        {
             ...
             * nicks, and BOTH will kill the other servers nick, which wont
             * exist,
        ...
        if(ConfigServerHide.hide_servers && MyClient(target_p) &&
           !IsOper(target_p))
            sendto_one(target_p, ":%s %s %s%s", me.name, numeric,
                       target_p->name, buffer);
        else
            sendto_one(target_p, ":%s %s %s%s",
                       ID_or_name(source_p, target_p->from), numeric,
                       ID_or_name(target_p, target_p->from), buffer);
        return;
    }
    else if((chptr = hash_find_channel(parv[1])) != NULL)
        sendto_channel_local(ALL_MEMBERS, NO, chptr, ":%s %s %s %s",
 ...
 */
void
m_not_oper(struct Client *client_p, struct Client *source_p, int parc,
           char *parv[])
{
...
void
m_unregistered(struct Client *client_p, struct Client *source_p, int parc,
               char *parv[])
{
    /* bit of a hack.
    ...
    {
        sendto_one(client_p, ":%s %d * %s :Register first.", me.name,
                   ERR_NOTREGISTERED, parv[0]);
        client_p->localClient->number_of_nick_changes++;
    }
}

void
m_registered(struct Client *client_p, struct Client *source_p, int parc,
             char *parv[])
{
...
void
m_ignore(struct Client *client_p, struct Client *source_p, int parc,
         char *parv[])
{
    return;
}
