...
parse_client_queued(struct Client *client_p)
{
    ...
    else if(IsClient(client_p))
    {
        if(ConfigFileEntry.no_oper_flood &&
           (IsOper(client_p) || IsCanFlood(client_p)))
        {
...
read_packet(fde_t *fd, void *data)
{
    ...
    do
    {
        ...
        if(IsServer(client_p) && IsPingSent(client_p) &&
           IsPingWarning(client_p))
        {
            char timestamp[200];
            strftime(timestamp, sizeof(timestamp), "%Y-%m-%d %H:%M:%S Z",
                     gmtime(&CurrentTime));
            sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                                 "Finally received packets from %s again after "
                                 "%d seconds (at %s)",
                                 get_client_name(client_p, SHOW_IP),
                                 CurrentTime - client_p->lasttime, timestamp);
        }
