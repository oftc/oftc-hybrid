...
static void chm_nosuch(struct Client *, struct Client *, struct Channel *, int,
                       int *, char **, int *, int, int, char, void *,
                       const char *);
static void chm_simple(struct Client *, struct Client *, struct Channel *, int,
                       int *, char **, int *, int, int, char, void *,
                       const char *);
static void chm_limit(struct Client *, struct Client *, struct Channel *, int,
                      int *, char **, int *, int, int, char, void *,
                      const char *);
...
static void chm_hop(struct Client *, struct Client *, struct Channel *, int,
                    ...
                    const char *);
#endif
static void chm_voice(struct Client *, struct Client *, struct Channel *, int,
                      int *, char **, int *, int, int, char, void *,
                      const char *);
static void chm_ban(struct Client *, struct Client *, struct Channel *, int,
                    int *, char **, int *, int, int, char, void *,
                    const char *);
static void chm_except(struct Client *, struct Client *, struct Channel *, int,
                       int *, char **, int *, int, int, char, void *,
                       const char *);
static void chm_invex(struct Client *, struct Client *, struct Channel *, int,
                      int *, char **, int *, int, int, char, void *,
                      const char *);
...
add_id(struct Client *client_p, struct Channel *chptr, char *banid, int type)
{
    ...
    if(MyClient(client_p))
    {
        ...
        {
            sendto_one(client_p, form_str(ERR_BANLISTFULL), me.name,
                       client_p->name, chptr->chname, banid);
            return 0;
        }
    ...
    {
        ban_p = ban->data;
        if(!irccmp(ban_p->name, name) && !irccmp(ban_p->username, user) &&
    ...
    {
        ban_p->who =
            MyMalloc(strlen(client_p->name) + strlen(client_p->username) +
                     strlen(client_p->host) + 3);
        ircsprintf(ban_p->who, "%s!%s@%s", client_p->name, client_p->username,
                   client_p->host);
    }
    else
...
del_id(struct Channel *chptr, char *banid, int type)
{
    ...
    DLINK_FOREACH(ban, list->head)
    {
        ...
        if(!irccmp(name, banptr->name) && !irccmp(user, banptr->username) &&
...
static const struct mode_letter
{
    ...
    {MODE_INVITEONLY, 'i'}, {MODE_MODERATED, 'm'},   {MODE_NOPRIVMSGS, 'n'},
    {MODE_PRIVATE, 'p'},    {MODE_SECRET, 's'},      {MODE_TOPICLIMIT, 't'},
    {MODE_NOCOLOR, 'c'},    {MODE_OPMODERATED, 'z'}, {MODE_SPEAKONLYIFREG, 'M'},
    {MODE_REGONLY, 'R'},    {MODE_SSLONLY, 'S'},     {0, '\0'}};
 ...
 */
void
channel_modes(struct Channel *chptr, struct Client *client_p, char *mbuf,
              char *pbuf)
{
    int i;
...
/* void god_mode_check(struct Client source_p, char *chname, int alev,
 * int parc, char **parv)
 *
 *  Inputs - source_p; The client who originated this mode.
 *         - chname; The channel this refers to.
 *         - alev; The access level pre determined
 *  Output - none
 *  Side-effects -
 *
 */
...
static void
god_mode_check(struct Client *source_p, char *chname, int alev, int parc,
               char **parv)
{
    ...
    ircsprintf(tmp, "%s is using God mode: MODE %s %s", source_p->name, chname,
               parv[0]);
    for(i = 1; i < parc; i++)
...
chm_nosuch(struct Client *client_p, struct Client *source_p,
           struct Channel *chptr, int parc, int *parn, char **parv, int *errors,
           int alev, int dir, char c, void *d, const char *chname)
{
...
static void
chm_ban(struct Client *client_p, struct Client *source_p, struct Channel *chptr,
        ...
        *errors |= SM_ERR_RPL_B;
        ...
        DLINK_FOREACH(ptr, chptr->banlist.head)
        {
            ...
            sendto_one(client_p, form_str(RPL_BANLIST), me.name, client_p->name,
                       chname, banptr->name, banptr->username, banptr->host,
           ...
           int alev, int dir, char c, void *d, const char *chname)
{
    ...
    if(dir == MODE_QUERY || parc <= *parn)
    {
        ...
        *errors |= SM_ERR_RPL_E;
        ...
        {
            const struct Ban *banptr = ptr->data;
            sendto_one(client_p, form_str(RPL_EXCEPTLIST), me.name,
                       client_p->name, chname, banptr->name, banptr->username,
                       banptr->host, banptr->who, banptr->when);
        }
          ...
          int alev, int dir, char c, void *d, const char *chname)
{
    ...
    if(!ConfigChannel.use_quiet && MyClient(source_p) && (dir == MODE_ADD) &&
       (parc > *parn))
    {
        if(*errors & SM_ERR_RPL_Q)
            return;
...
static void
chm_op(struct Client *client_p, struct Client *source_p, struct Channel *chptr,
       int parc, int *parn, char **parv, int *errors, int alev, int dir, char c,
...
#ifdef HALFOPS
static void
chm_hop(struct Client *client_p, struct Client *source_p, struct Channel *chptr,
        int parc, int *parn, char **parv, int *errors, int alev, int dir,
        char c, void *d, const char *chname)
{
    char *opnick;
...
static void
chm_key(struct Client *client_p, struct Client *source_p, struct Channel *chptr,
        int parc, int *parn, char **parv, int *errors, int alev, int dir,
        char c, void *d, const char *chname)
{
    int i;
                      ...
                      struct Channel *chptr, int cap, int nocap)
{
    ...
    for(i = 0; i < mode_count; i++)
    {
        ...
        if((mc == MAXMODEPARAMS) || ((arglen + mbl + pbl + 2) > IRCD_BUFSIZE) ||
                  ...
                  struct Channel *chptr, char *chname)
{
    ...
    for(i = 0; i < mode_count; i++)
    {
        if(mode_changes[i].letter == 0 || mode_changes[i].mems == NON_CHANOPS ||
    ...
    if(mode_count > 20)
      break;
        ...
        default:
            ...
            ModeTable[table_position].func(client_p, source_p, chptr, parc,
                                           &parn, parv, &errors, alevel, dir, c,
