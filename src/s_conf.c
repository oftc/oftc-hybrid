...
map_to_conf(struct ConfItem *aconf)
{
    void *conf;
    conf =
        (void *)((unsigned long)aconf + (unsigned long)sizeof(struct ConfItem));
    return (conf);
}
...
static const unsigned int shared_bit_table[] = {'K', 'k', 'U', 'X', 'x', 'Y',
                                                'Q', 'q', 'R', 'L', 0};
    ...
    case GDENY_TYPE:
        DLINK_FOREACH(ptr, gdeny_items.head)
        {
            ...
            *p = '\0';
            ...
            sendto_one(source_p, ":%s %d %s V %s@%s %s %s", me.name,
                       RPL_STATSDEBUG, source_p->name, aconf->user, aconf->host,
                       conf->name, buf);
        }
        break;
    case XLINE_TYPE:
        DLINK_FOREACH(ptr, xconf_items.head)
        {
            ...
            sendto_one(source_p, form_str(RPL_STATSXLINE), me.name,
                       source_p->name, matchitem->hold ? "x" : "X",
                       matchitem->count, conf->name, matchitem->reason);
        }
        break;
    case RXLINE_TYPE:
        DLINK_FOREACH(ptr, rxconf_items.head)
        {
            ...
            sendto_one(source_p, form_str(RPL_STATSXLINE), me.name,
                       source_p->name, matchitem->hold ? "xR" : "XR",
                       matchitem->count, conf->name, matchitem->reason);
        }
        break;
    case RKLINE_TYPE:
        ...
        DLINK_FOREACH(ptr, oconf_items.head)
        {
            ...
            if(IsOper(source_p))
            {
                sendto_one(source_p, form_str(RPL_STATSOLINE), me.name,
                           source_p->name, 'O', aconf->user, aconf->host,
                           conf->name, oper_privs_as_string(aconf->port),
                           aconf->class_ptr ? aconf->class_ptr->name
                                            : "<default>");
            }
            else
            {
                sendto_one(
                    source_p, form_str(RPL_STATSOLINE), me.name, source_p->name,
                    'O', aconf->user, aconf->host, conf->name, "0",
    ...
    case CLASS_TYPE:
        DLINK_FOREACH(ptr, class_items.head)
        {
            ...
            sendto_one(source_p, form_str(RPL_STATSYLINE), me.name,
                       source_p->name, 'Y', conf->name, PingFreq(classitem),
                       ConFreq(classitem), MaxTotal(classitem),
                       MaxSendq(classitem), CurrUserCount(classitem),
    ...
    case SERVER_TYPE:
        DLINK_FOREACH(ptr, server_items.head)
        {
            ...
            *p = '\0';
            ...
            {
                sendto_one(
                    source_p, form_str(RPL_STATSCLINE), me.name, source_p->name,
                    'C', aconf->host, buf, conf->name, aconf->port,
                    aconf->class_ptr ? aconf->class_ptr->name : "<default>");
            }
            else
            {
                sendto_one(
                    source_p, form_str(RPL_STATSCLINE), me.name, source_p->name,
                    'C', "*@127.0.0.1", buf, conf->name, aconf->port,
                    aconf->class_ptr ? aconf->class_ptr->name : "<default>");
            }
        }
        break;
...
check_client(va_list args)
{
    ...
    case MAX_IDENT:
        sendto_gnotice_flags(
            UMODE_FULL, L_ALL, me.name, &me, NULL, full_reasons[i],
            get_client_name(source_p, SHOW_IP), source_p->sockhost, conf.name);
        ilog(L_INFO, full_reasons[i], get_client_name(source_p, SHOW_IP),
    ...
    case NOT_AUTHORIZED:
    {
        ...
        irc_getnameinfo((struct sockaddr *)&source_p->ip, source_p->ip.ss_len,
                        ipaddr, HOSTIPLEN, NULL, 0, NI_NUMERICHOST);
        sendto_gnotice_flags(
            UMODE_UNAUTH, L_ALL, me.name, &me, NULL,
              ...
              struct ConfItem *retconf, char **reason)
{
    ...
    if(IsGotId(client_p))
    {
        aconf = find_address_conf(
            client_p->host, client_p->username, &client_p->ip, client_p->aftype,
    ...
    else
    {
        ...
        aconf = find_address_conf(
            client_p->host, non_ident, &client_p->ip, client_p->aftype,
            client_p->localClient->passwd, client_p->certfp);
    }
    ...
    if(aconf != NULL)
    {
        if(IsConfClient(aconf) && !rkconf)
        {
            ...
            {
                sendto_one(client_p, form_str(RPL_REDIR), me.name,
                           client_p->name, conf->name ? conf->name : "",
                           aconf->port);
                ...
                (ConfigFileEntry.glines && IsConfGline(aconf)))
        {
            ...
            {
                sendto_one(client_p, ":%s NOTICE %s :*** Banned %s", me.name,
                           client_p->name, aconf->reason);
            }
...
attach_iline(struct Client *client_p, struct ConfItem *conf, char **reason)
{
    ...
    count_user_host(client_p->username, client_p->host, &global, &local,
                    &ident);
    ...
    else if(cidr_limit_reached(IsConfExemptLimits(aconf), &client_p->ip,
                               aclass))
    {
...
{
    ip_entry_heap =
        BlockHeapCreate("ip", sizeof(struct ip_entry), 2 * hard_fdlimit);
...
find_or_add_ip(struct irc_ssaddr *ip_in)
{
    ...
    for(ptr = ip_hash_table[hash_index]; ptr; ptr = ptr->next)
    {
        ...
        if(ip_in->ss.ss_family == AF_INET6)
        {
            ...
            res    = memcmp(&v6->sin6_addr, &ptr_v6->sin6_addr,
...
#endif
        {
            ...
            res    = memcmp(&v4->sin_addr, &ptr_v4->sin_addr,
    ...
    for(i = 0; i < IP_HASH_SIZE; i++)
    {
        for(ptr = ip_hash_table[i]; ptr != NULL; ptr = ptr->next)
        {
            ...
            sendto_one(source_p, ":%s %d %s n :ip_hash_table: %s %d", me.name,
                       ...
                       (ret == 0) ? numaddr : "unknown", ptr->count);
        }
    }
}
    ...
    case NRESV_TYPE:
        DLINK_FOREACH(ptr, list_p->head)
        {
            ...
            if((name != NULL) && match_esc(conf->name, name))
            {
                ...
                if(EmptyString(match_item->user) ||
                   EmptyString(match_item->host))
                {
                    return conf;
                }

                if(match(match_item->user, user) &&
                   match(match_item->host, host))
                {
                    return conf;
                }
            }
 ...
 */
struct ConfItem *
find_exact_name_conf(ConfType type, const char *name, const char *user,
                     const char *host, const char *certfp)
{
    dlink_node *ptr = NULL;
    struct AccessItem *aconf;
    struct ConfItem *conf;
    ...
    case OPER_TYPE:
        DLINK_FOREACH(ptr, list_p->head)
        {
            ...
            if(irccmp(conf->name, name) == 0)
            {
                ...
                {
                    if(strncmp(aconf->certfp, certfp, SHA_DIGEST_LENGTH * 2) ==
                       0)
                    {
                        return conf;
                    }
...
lookup_confhost(struct ConfItem *conf)
{
    ...
    {
        ilog(L_ERROR, "Host/server name error: (%s) (%s)", aconf->host,
             conf->name);
        return;
    }
...
find_kill(struct Client *client_p)
{
    ...
    {
        aconf =
            find_kline_conf(client_p->realhost, client_p->username,
    ...
    {
        aconf =
            find_kline_conf(client_p->host, client_p->username,
                            client_p->certfp, &client_p->ip, client_p->aftype);
    }
...
find_gline(struct Client *client_p)
{
    ...
    aconf = find_gline_conf(client_p->host, client_p->username, &client_p->ip,
                            client_p->aftype);
...
expire_tklines(dlink_list *tklist)
{
    ...
    {
        conf = ptr->data;
        if(conf->type == GLINE_TYPE || conf->type == KLINE_TYPE ||
        ...
        else if(conf->type == XLINE_TYPE || conf->type == RXLINE_TYPE)
        {
            ...
            if(xconf->hold <= CurrentTime)
            {
                ...
                {
                    sendto_realops_flags(
                        UMODE_ALL, L_ALL, "Temporary X-line for [%s] %sexpired",
                        conf->name,
...
} flag_list[] = {{OPER_FLAG_ADMIN, OPER_FLAG_HIDDEN_ADMIN, 'A'},
...
get_oper_name(const struct Client *client_p)
{
    ...
    ircsprintf(buffer, "%s!%s@%s{%s}", client_p->name, client_p->username,
...
read_conf_files(int cold)
{
    ...
    if((conf_fbfile_in = fbopen(filename, "r")) == NULL)
    {
        if(cold)
        {
            ilog(L_CRIT, "Unable to read configuration file '%s': %s", filename,
                 strerror(errno));
            exit(-1);
        }
        else
    ...
    ircsprintf(chanmodes, "%s%s%s%s", ConfigChannel.use_except ? "e" : "",
               ...
               ConfigChannel.use_quiet ? "q" : "", "b,k,l,cimnpstzMRS");
...
clear_out_old_conf(void)
{
    ...
    struct MatchItem *match_item;
    dlink_list *free_items[] = {&server_items,  &oconf_items,  &hub_items,
                                &leaf_items,    &uconf_items,  &xconf_items,
                                &rxconf_items,  &rkconf_items, &nresv_items,
                                &cluster_items, &gdeny_items,  NULL};
...
flush_deleted_I_P(void)
{
    ...
    struct ConfItem *conf;
    struct AccessItem *aconf;
    dlink_list *free_items[] = {&server_items, &oconf_items, NULL};
    dlink_list **iterator    = free_items; /* C is dumb */
...
conf_add_class_to_conf(struct ConfItem *conf, const char *class_name)
{
    ...
    if(class_name == NULL)
    {
        ...
        else
        {
            sendto_gnotice_flags(
                UMODE_ALL, L_ALL, me.name, &me, NULL,
                "Warning *** Defaulting to default class for %s", conf->name);
        }
    }
    else
    {
...
conf_add_server(struct ConfItem *conf, const char *class_name)
{
    ...
    DupString(aconf->user,
              conf_user); /* somehow username checking for servers
...
yyerror(const char *msg)
{
    ...
    sendto_gnotice_flags(UMODE_ALL, L_ALL, me.name, &me, NULL,
                         "\"%s\", line %u: %s: %s", conffilebuf, lineno + 1,
                         msg, newlinebuf);
    ilog(L_WARN, "\"%s\", line %u: %s: %s", conffilebuf, lineno + 1, msg,
         newlinebuf);
}
 ...
 */
int
parse_aline(const char *cmd, struct Client *source_p, int parc, char **parv,
            ...
            char **target_server, char **reason)
{
    ...
    if(parc != 0)
    {
        if(irccmp(*parv, "ON") == 0)
        {
            ...
            {
                sendto_one(source_p,
                           ":%s NOTICE %s :ON server not supported by %s",
                           me.name, source_p->name, cmd);
                return -1;
            }
    ...
    if(h_p != NULL)
    {
        ...
        {
            sendto_one(source_p,
                       ":%s NOTICE %s :Invalid character '!' in kline", me.name,
                       source_p->name);
            return -1;
        }

        if((parse_flags & AWILD) &&
           !valid_wild_card(source_p, YES, 2, *up_p, *h_p))
        {
            return -1;
        }
    }
 ...
 */
static int
find_user_host(struct Client *source_p, char *user_host_or_nick, char *luser,
               char *lhost, unsigned int flags)
{
    ...
    if((hostp = strchr(user_host_or_nick, '@')) || *user_host_or_nick == '*')
    {
        ...
        if(hostp != NULL) /* I'm a little user@host */
        {
            *(hostp++) = '\0'; /* short and squat */
            ...
            {
                strlcpy(luser, user_host_or_nick,
                        USERLEN * 4 + 1); /* here is my user */
            }
            else
            {
                strcpy(luser, "*");
    ...
    else if(!(flags & NOUSERLOOKUP))
    {
        ...
        if(IsExemptKline(target_p))
        {
            ...
            {
                sendto_one(source_p, ":%s NOTICE %s :%s is E-lined", me.name,
 ...
 */
int
cidr_limit_reached(int over_rule, struct irc_ssaddr *ip,
                   struct ClassItem *aclass)
{
    dlink_node *ptr = NULL;
...
rebuild_cidr_class(struct ConfItem *conf, struct ClassItem *new_class)
{
    ...
    if(NumberPerCidr(old_class) > 0 && NumberPerCidr(new_class) > 0)
    {
        if(CidrBitlenIPV4(old_class) > 0 && CidrBitlenIPV4(new_class) > 0)
            rebuild_cidr_list(AF_INET, conf, new_class, &old_class->list_ipv4,
                              &new_class->list_ipv4,
                              CidrBitlenIPV4(old_class) !=
                                  CidrBitlenIPV4(new_class));

#ifdef IPV6
        if(CidrBitlenIPV6(old_class) > 0 && CidrBitlenIPV6(new_class) > 0)
            rebuild_cidr_list(AF_INET6, conf, new_class, &old_class->list_ipv6,
                              &new_class->list_ipv6,
                              CidrBitlenIPV6(old_class) !=
                                  CidrBitlenIPV6(new_class));
#endif
    }
